	...
	public void pageView(String url, String params) {
		...
		if (this.pageNo != 1) {
			pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=1'\"><font size=3>首页</font></a>");
			pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=" + (this.pageNo - 1) + "'\"><font size=3>上一页</font></a>");
		} else {
			pageView.add("<span class='disabled'><font size=3>首页</font></span>");
			pageView.add("<span class='disabled'><font size=3>上一页</font></span>");
		}

		if (this.getTotalPage() <= 10) {
			for (int i = 0; i < this.getTotalPage(); i++) {
				if ((i + 1) == this.pageNo) {
					pageView.add("<span class='current'>" + this.pageNo + "</span>");
					i = i + 1;
		...
		if (this.pageNo != this.getTotalPage()) {
			pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=" + (this.pageNo + 1) + "'\"><font size=3>下一页</font></a>");
			pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=" + this.getTotalPage() + "'\"><font size=3>尾页</font></a>");
		} else {
			pageView.add("<span class='disabled'><font size=3>下一页</font></span>");
			pageView.add("<span class='disabled'><font size=3>尾页</font></span>");
		}
		pageView.add("<font size=3>" + this.pageNo + "/" + getTotalPage() + " , 共有" + this.totalCount + "条数据。</font> ");
	}
}
