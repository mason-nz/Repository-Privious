...
using XYAuto.BUOC.BOP2017.ExternalApi.GDT.Dto.Response.OAuth;
...
namespace XYAuto.BUOC.BOP2017.ExternalApi.GDT
{
    public class ServiceHelper
    {
        #region Instance

        private static readonly Infrastruction.Http.DoHttpClient DoHttpClient = new DoHttpClient();
        ...
        public string AddAdvertiser_Test(string corporation_name, string website)
        {
            ...
            HttpWebResponse hwr = HttpHelper.CreatePostHttpResponse(GDT_URL + GDT_API_VERSION + url, paras, null, (int)RequestContentType.JSON);
            string msg = HttpHelper.GetResponseString(hwr);
            return msg;
        }
        ...
        public string AddImagesInfo(int accountId)
        {
            ...
            paras.Add("signature", Util.GetMD5HashFromFile(imagePath));

            return HttpHelper.CreatePostHttpResponseByMultipart(GDT_URL + GDT_API_VERSION + url, 10000, "file", imagePath, paras);
        }
        ...
        public string GetImagesInfo(int account_id)
        {
            ...
            HttpWebResponse hwr = HttpHelper.CreateGetHttpResponse(GDT_URL + GDT_API_VERSION + url);
            string msg = HttpHelper.GetResponseString(hwr);
            return msg;
        }
        ...
        public string AddCampaigns_Test(int account_id, string campaign_name, string campaign_type)
        {
            ...
            HttpWebResponse hwr = HttpHelper.CreatePostHttpResponse(GDT_URL + GDT_API_VERSION + url, paras, null, (int)RequestContentType.JSON);
            return HttpHelper.GetResponseString(hwr);
        }
        ...
        public RespPageInfo<List<RespAccountInfoDto>> GetAdvertiserInfo(ReqReportDto reportDto)
        {
            ...
            var result = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<RespAccountInfoDto>>>>
                (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        public List<RespFundDto> GetFundsInfo(int accountId)
        {
            ...
            var result = new DoPostApiLogClient(url, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<RespFundDto>>>>
               (s => DoHttpClient.Get(url).Result, Loger.GdtLogger.Info);
        ...
        public RespPageInfo<List<RespRealtimeCostDto>> GetRealtimeCost(ReqReportDto reportDto)
        {
            ...
            var result = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<RespRealtimeCostDto>>>>
                  (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        public List<FundStatementsDailyDto> GetFundStatementsDaily(ReqFundDto fundDto)
        {
            ...
            var result = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<FundStatementsDailyDto>>>>
                 (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        public RespPageInfo<List<FundStatementsDailyDto>> GetFundStatementsDetailed(ReqFundDetaileDto fundDetaileDto)
        {
            ...
            requestUrl = GDT_URL + GDT_API_VERSION + requestUrl;
            var baseResult = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<FundStatementsDailyDto>>>>
                  (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        private RespTokenDto GetAccessToken(GdtGrantTypeEnum grantType, string authorizationCode, string refreshToken, string redirectUri)
        {
            ...
            var baseResult = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespTokenDto>>
                 (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        public RespPageInfo<List<RespReportDto>> GetReportDaily(ReqReportDto reportDto)
        {
            var requestUrl = $"{GetCommonURL("/daily_reports/get")}&level={reportDto.Level}&account_id={reportDto.AccountId}" +
                             $"&date_range={JsonConvert.SerializeObject(reportDto.DateRange)}" +
                             $"&page={reportDto.Page}&page_size={reportDto.PageSize}";
            ...
            requestUrl = GDT_URL + GDT_API_VERSION + requestUrl;
            var baseResult = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<RespReportDto>>>>
                    (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        public RespPageInfo<List<RespAdGroupDto>> GetAdGroupList(ReqReportDto reportDto)
        {
            ...
            if (reportDto.Filtering != null && reportDto.Filtering.Count > 0)
                requestUrl += $"&filtering={ JsonConvert.SerializeObject(reportDto.Filtering)}";
            requestUrl = GDT_URL + GDT_API_VERSION + requestUrl;
            var baseResult = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<RespAdGroupDto>>>>
                   (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        public RespPageInfo<List<RespAdCampaignDto>> GetAdCampaignsList(ReqReportDto reportDto)
        {
            ...
            requestUrl = GDT_URL + GDT_API_VERSION + requestUrl;
            var baseResult = new DoPostApiLogClient(requestUrl, string.Empty).GetPostResult<Dto.Response.RespBaseDto<RespPageInfo<List<RespAdCampaignDto>>>>
                   (s => DoHttpClient.Get(requestUrl).Result, Loger.GdtLogger.Info);
        ...
        private string GetCommonURL(string url)
        {
            if (!string.IsNullOrWhiteSpace(url))
            {
                ...
                url += "&timestamp=" + Util.ConvertDateTimeInt(DateTime.Now);
