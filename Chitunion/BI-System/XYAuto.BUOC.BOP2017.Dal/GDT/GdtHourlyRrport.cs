...
namespace XYAuto.BUOC.BOP2017.Dal.GDT
{
    ...
    public partial class GdtHourlyRrport : DataBase
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Insert(Entities.GDT.GdtHourlyRrport entity)
        {
        ...
        public int GetMaxHour(Entities.GDT.GdtHourlyRrport entityWhere)
        {
            var sql = string.Format(@"SELECT ISNULL(MAX(Hour),-1) FROM DBO.GDT_HourlyRrport
                                            WHERE AccountId = {0}
                                            AND Level = {1}
                                            AND [Date] = '{2}'
                                            AND AdgroupId = {3}
                                            ", entityWhere.AccountId, entityWhere.Level,
                                             entityWhere.Date.ToString("yyyy-MM-dd"), entityWhere.AdgroupId);
