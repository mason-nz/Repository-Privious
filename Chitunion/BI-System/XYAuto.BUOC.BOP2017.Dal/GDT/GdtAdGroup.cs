...
namespace XYAuto.BUOC.BOP2017.Dal.GDT
{
    public class GdtAdGroup : DataBase
    {
        ...
        public List<Entities.GDT.GdtAdGroup> GetList(int accountId, int configuredStatus, int systemStatus)
        {
            var sql = @"
            ...
            if (accountId > 0)
            {
                sql += $" AND ADS.AccountId = {accountId}";
            }
            ...
            if (systemStatus > 0)
            {
                sql += $" AND ADS.SystemStatus = {systemStatus}";
            }
