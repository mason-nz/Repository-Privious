        ...
        public GetDemandDetailResDTO GetDemandDetail(int demandBillNo, int yyUserID)
        {
            ...
            string sql = $@"SELECT  d.DemandId ,
	                                ...
	                                FROM    dbo.GDT_Demand d
			                                      ...
			                                      LEFT JOIN Chitunion2017.dbo.UserInfo ui1 ON d.CreateUserId = ui1.UserID
                                                  LEFT JOIN dbo.GDT_RoleUser ru on d.CreateUserId = ru.UserID
                                                  LEFT JOIN Chitunion2017.dbo.UserInfo ui2 ON ru.AuthToUserId = ui2.UserID
...
ISNULL(CAST((CAST(SUM(hr.Click) AS  decimal(18, 4))/CAST(SUM(hr.Impression) AS  decimal(18, 4))) AS DECIMAL(18,4)),0) AS AvgClickPercent,
        ...
        public bool AuditDemand(int demandBillNo, int auditStatus, string rejectReason, int auditUserID)
        {
            ...
            using (SqlConnection conn = new SqlConnection(CONNECTIONSTRINGS))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        if (auditStatus == (int)Entities.Enum.GDT.DemandAuditStatusEnum.Terminated || auditStatus == (int)Entities.Enum.GDT.DemandAuditStatusEnum.IsOver)
                        {
        ...
        public List<ADGroupDTO> GetCanSelectAdGroupList(int deliveryId, int demandBillNo)
        {//有效的 系统状态
            var sql = $@"
                    ...
                    SELECT  ag.AdgroupId ,
                            ...
                            ag.ConfiguredStatus ,
                            c.CampaignName ,
                            CAST(( CASE WHEN dr.DemandBillNo IS NULL THEN 0
                                        ELSE 1
                                   END ) AS BIT) AS IsChecked ,
                            dr.DeliveryId
                    FROM    dbo.GDT_AdGroup ag
                            LEFT JOIN dbo.GDT_DemandRelation dr ON ag.AdgroupId = dr.AdgroupId
		                    --LEFT JOIN DBO.Demand_GroundDelivery AS GD WITH(NOLOCK) ON GD.DeliveryId = dr.DeliveryId
                    ...
                    WHERE   ag.SystemStatus = 87001
                            AND ( dr.DeliveryId IS NULL
                                  OR dr.DeliveryId = {deliveryId}
                                )
                    ";
        ...
        public MapGetRightOneResDTO GetAccountInfo(int userID)
        {
            ...
            string sql = $@"SELECT ab.FundType,ISNULL(SUM(ab.Balance),0) AS 'Current',ISNULL(SUM(ab.RealtimeCost),0) AS 'Cost' FROM dbo.GDT_AccountInfo ai
		                              INNER JOIN dbo.GDT_AccountBalance ab ON ai.AccountId = ab.AccountId AND ab.FundType IN (81001,81002,81003,81004)
                                       -- AND ab.FundStatus = 82001
        ...
        public MapGetRightTwoResDTO GetStatistic(int demandBillNo, int adGroupID, int userID, string beginTime,
             string endTime)
        {
            MapGetRightTwoResDTO res = new MapGetRightTwoResDTO();
            //曝光 点击 平均点击 订单 话单
            //string sql = $@"SELECT
            //                          ISNULL(SUM(hr.Cost),0) AS TotalCost,
            ...
            string sql = $@" SELECT *,  CASE WHEN ClueCount = 0 THEN 0 ELSE ISNULL(CAST((CAST(TotalCost AS  decimal(18, 2))/CAST(ClueCount AS  decimal(18, 2))) AS DECIMAL(18,2)),0)  END AS CluePrice FROM (SELECT
                                      ISNULL(SUM(hr.Cost),0) AS TotalCost,
                                      ISNULL(SUM(hr.Impression),0) AS TotalImpression,
                                      ISNULL(SUM(hr.Click),0) AS TotalClick,
                                     ISNULL(CAST((CAST(SUM(hr.Click) AS  decimal(18, 4))/CAST(SUM(hr.Impression) AS  decimal(18, 4))) AS DECIMAL(18,4)),0) AS AvgClickPercent,
                                      ISNULL(SUM(hr.OrderQuantity),0) AS OrderQuantity,
                                      ISNULL(SUM(hr.BillOfQuantities),0) AS BillOfQuantities FROM dbo.GDT_HourlyRrport hr
                                      INNER JOIN dbo.GDT_AccountRelation ar ON hr.AccountId = ar.AccountId
                                      ...
                                      AND ({demandBillNo} = 0 OR dr.DemandBillNo = {demandBillNo})
                                      AND ({adGroupID} = 0 OR dr.AdgroupId = {adGroupID})
                                      AND hr.Date >='{beginTime}' AND hr.Date <'{endTime}') A,
                                      (
                                      SELECT
                                      ISNULL(SUM(hpv.PV),0) AS PV,ISNULL(SUM(hpv.UV),0) AS UV,ISNULL(SUM(hpv.ClueCount),0) AS ClueCount
                                      FROM  dbo.GDT_HourlyPageView hpv
                                      INNER JOIN dbo.Demand_GroundDelivery gd ON gd.PromotionUrl = hpv.AdPage
                                      INNER JOIN dbo.GDT_DemandRelation dr ON gd.DeliveryId = dr.DeliveryId AND gd.DemandBillNo = dr.DemandBillNo
                                      INNER JOIN dbo.GDT_AdGroup ag ON ag.AdgroupId = dr.AdgroupId
                                      INNER JOIN dbo.GDT_AccountRelation ar ON ar.AccountId = ag.AccountId
                                      INNER JOIN dbo.GDT_RoleUser ru ON ar.UserId = ru.UserId
                                      WHERE ({userID} = 0 OR ru.AuthToUserId = {userID})
                                      AND ({demandBillNo} = 0 OR dr.DemandBillNo = {demandBillNo})
                                      AND ({adGroupID} = 0 OR dr.AdgroupId = {adGroupID})
                                      AND hpv.Date >='{beginTime}' AND hpv.Date <'{endTime}'
                                      ) B";
            var dt = SqlHelper.ExecuteDataset(CONNECTIONSTRINGS, CommandType.Text, sql).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //二期新增
                res.PV = (int)dt.Rows[0]["PV"];
                res.UV = (int)dt.Rows[0]["UV"];
                res.ClueCount = (int)dt.Rows[0]["ClueCount"];
                res.CluePrice = (decimal)dt.Rows[0]["CluePrice"];
            }
            if (adGroupID != 0)
            {//广告详情
                sql = $@"SELECT
                                gd.PromotionUrl,
                                ag.AdgroupName,ui.UserName AS BelongYY,di.DictName AS AuditStatusName FROM dbo.GDT_AdGroup ag
                                INNER JOIN dbo.GDT_DemandRelation dr ON ag.AdgroupId = dr.AdgroupId
                                INNER JOIN dbo.GDT_AccountRelation ar ON ag.AccountId = ar.AccountId
                                LEFT JOIN dbo.GDT_RoleUser ru ON ar.UserId = ru.UserId
                                INNER JOIN Chitunion2017.dbo.UserInfo ui ON ru.AuthToUserId = ui.UserID
                                LEFT JOIN dbo.DictInfo di ON ag.SystemStatus = di.DictId
                                LEFT JOIN dbo.Demand_GroundDelivery gd on dr.DeliveryId = gd.DeliveryId
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    res.AdPage = dt.Rows[0]["PromotionUrl"].ToString();
                }
            }
        ...
        public List<MapGetRightThreeItemDTO> GetChart(int demandBillNo, int adGroupID, DateTime startDt, DateTime endDt, int dataType, int userID)
        {
            ...
            string sql = $@" SELECT * FROM ( SELECT  hr.*,gd.PromotionUrl  FROM dbo.GDT_HourlyRrport hr
                                      INNER JOIN dbo.GDT_DemandRelation dr ON hr.AdgroupId = dr.AdgroupId
                                      INNER JOIN dbo.GDT_AccountRelation ar ON hr.AccountId = ar.AccountId
                                      LEFT JOIN dbo.GDT_RoleUser ru ON ar.UserId = ru.UserId
                                      LEFT JOIN dbo.Demand_GroundDelivery gd ON dr.DeliveryId = gd.DeliveryId
                                      ...
                                      AND ({demandBillNo} = 0 OR dr.DemandBillNo = {demandBillNo})
                                      AND ({adGroupID} = 0 OR dr.AdgroupId = {adGroupID})
                                      AND hr.Date>= @StartDate AND hr.Date<@EndDate ) A 
                                      LEFT JOIN (SELECT * FROM dbo.GDT_HourlyPageView hpv
                                      INNER JOIN dbo.Demand_GroundDelivery gd ON gd.PromotionUrl = hpv.AdPage
                                      AND hpv.Date>= @StartDate AND hpv.Date< @EndDate) B
                                      ON A.Hour = B.Hour AND A.PromotionUrl = B.AdPage AND A.Date = B.Date";
            SqlParameter[] parameters = new SqlParameter[]
            {
                ...
                list.GroupBy(i => new { i.Date, i.Hour }).OrderBy(gb => gb.Key.Date).ThenBy(gb => gb.Key.Hour).ToList().ForEach(gb =>
                    {
                        ...
                        switch (dataType)
                        {//曝光量:1 点击量:2 点击率:3 费用:4 订单量:5 话单量:6   PV:7 UV:8 线索量：9 线索单价：10
                            case 1:
                            ...
                            case 7:
                                item.Value = gb.Sum(i => i.PV);
                                break;

                            case 8:
                                item.Value = gb.Sum(i => i.UV);
                                break;

                            case 9:
                                item.Value = gb.Sum(i => i.ClueCount);
                                break;

                            case 10:
                                if (gb.Sum(i => i.ClueCount) == 0)
                                {
                                    item.Value = 0.0M;
                                }
                                else
                                {
                                    item.Value = Math.Round((decimal)gb.Sum(i => i.Cost) / gb.Sum(i => i.ClueCount),2);
                                }
                                break;
                        }
                        res.Add(item);
        ...
        public MapGetRightADListResDTO GetADGroupDetailList(int demandBillNo, int adGroupID, string startDt, string endDt, int userID, int pageIndex, int pageSize)
        {
            ...
            if (res.List != null && res.List.Count > 0)
            {
                ...
                res.List.ForEach(item => { item.AdgroupDate = item.Date.ToString("yyyy-MM-dd") + $" {item.Hour}-{item.Hour + 1}时"; });
            }
            return res;
        }
        ...
        public List<ADGroupDTO> GetAllADGroupDetailList(int demandBillNo, int adGroupID, string startDt, string endDt, int userID)
        {
            ...
            string sql = $@" SELECT A.*,B.PV,B.UV,B.ClueCount,CASE WHEN ClueCount = 0 THEN 0 ELSE ISNULL(CAST((TotalCost/ClueCount) AS DECIMAL(18,2)),0)  END AS CluePrice FROM ( SELECT hr.Date ,
		                                           hr.Hour ,
		                                           SUM(hr.Impression) AS TotalImpression ,
		                                           SUM(hr.Click) AS TotalClick ,
		                                           CAST(( CAST(SUM(hr.Click) AS  decimal(18, 4)) / CAST(SUM(hr.Impression) AS  decimal(18, 4)) ) AS DECIMAL(18, 4)) AS AvgClickPercent ,
		                                           SUM(hr.Cost) AS TotalCost ,
		                                           SUM(hr.OrderQuantity) AS OrderQuantity ,
		                                           SUM(hr.BillOfQuantities) AS BillOfQuantities
	                                  FROM   dbo.GDT_HourlyRrport hr
            ...
            sql += "  GROUP BY hr.Date ,hr.Hour ";
            sql += $" ) A LEFT JOIN (SELECT hpv.Date, hpv.Hour,ISNULL(SUM(hpv.PV), 0) AS PV, ISNULL(SUM(hpv.UV), 0) AS UV, ISNULL(SUM(hpv.ClueCount), 0) AS ClueCount FROM dbo.GDT_HourlyPageView hpv INNER JOIN dbo.Demand_GroundDelivery gd ON gd.PromotionUrl = hpv.AdPage INNER JOIN dbo.GDT_DemandRelation dr ON gd.DeliveryId = dr.DeliveryId AND gd.DemandBillNo = dr.DemandBillNo INNER JOIN dbo.GDT_AdGroup ag ON ag.AdgroupId = dr.AdgroupId INNER JOIN dbo.GDT_AccountRelation ar ON ar.AccountId = ag.AccountId INNER JOIN dbo.GDT_RoleUser ru ON ar.UserId = ru.UserId WHERE ({userID} = 0 OR ru.AuthToUserId = {userID}) AND({ demandBillNo} = 0 OR dr.DemandBillNo = { demandBillNo})AND({ adGroupID} = 0 OR dr.AdgroupId = {adGroupID}) ";
            if (!string.IsNullOrEmpty(startDt) || !string.IsNullOrEmpty(endDt))
            {
                sql += $" AND hpv.Date BETWEEN '{startDt}' AND '{endDt}'";
            }
            sql += $"  GROUP BY hpv.Date ,hpv.Hour) B ON A.Date = B.Date AND A.Hour = B.Hour";
            sql += " ORDER BY A.Date DESC, A.Hour DESC";
            List<ADGroupDTO> list = new List<ADGroupDTO>();
            var dt = SqlHelper.ExecuteDataset(CONNECTIONSTRINGS, CommandType.Text, sql).Tables[0];
        ...
        public DataTable SelectDemandRechargeInfo(int demandBillNo)
        {
            string strSql = $"SELECT DemandBillNo,RechargeNumber,A.AccountId FROM GDT_RechargeRelation R INNER JOIN  GDT_AccountRelation A ON R.CreateUserId=A.UserId WHERE R.DemandBillNo={demandBillNo}";
            return SqlHelper.ExecuteDataset(CONNECTIONSTRINGS, CommandType.Text, strSql).Tables[0];
        }

        public int InsertAbnormalFunds(DemandRefundInfo drInfo)
        {
            string strSql = $@"insert into GDT_AbnormalFunds (RechargeNumber,DemandBillNo,AccountId,TradeType,RechargeAmount,SpendAmount,
                             TransferBackAmount,CreateTime,CreateUserId) VALUES ('{drInfo.RechargeNumber}',{drInfo.DemandBillNo},{drInfo.AccountId},{(int)drInfo.TradeType}
                             ,{drInfo.RechargeAmount},{drInfo.SpendAmount},{drInfo.TransferBackAmount},'{drInfo.CreateTime}',{drInfo.CreateUserId})";
