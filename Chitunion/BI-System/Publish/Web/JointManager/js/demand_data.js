$(function() {
      ...
      //渲染左侧信息
      $.ajax({
            url:urlN+"/api/Demand/MapGetLeft",
            type:"get",
            xhrFields:{
      ...
      //右上部分数据
      $.ajax({
            url:urlN+"/api/Demand/MapGetRightTwo",
            type:"get",
            dataType:"json",
            data:{
      ...
      //中间的需求详情
      $.ajax({
            url:urlN+"/api/Demand/GetDemandDetail",
            type:"get",
            dataType:"json",
            xhrFields:{
                  withCredentials:true
    ...
    //右部分图表逻辑交互（默认加载今天和点击量的数据）
      var DateType,
         ...
         function get_data(){
               $.ajax({
                     url:urlN+"/api/Demand/MapGetRightThree",
                     type:"get",
                     dataType:"json",
                     xhrFields:{
                           withCredentials:true
         ...
         //获取图表option参数的函数
             function get_option(){
                   ...
                   if(echarts_name == "点击量" || echarts_name == "曝光量"){
                         ...
                         y_name = "单位：%";
                   }else if(echarts_name == "费用" || echarts_name=="线索单价"){
                         y_name = "单位：元";
                         y_data = y_data.map(function(a){
                            return a/100;
                         })
                         y_max = y_max/100;
                   }else if(echarts_name == "PV" || echarts_name == "UV" || echarts_name == "线索量"){
                         y_name = "单位：个";
                   }
          ...
          $(".echarts_data ul .special").off("click").on("click",function() {
            ...
            function get_data_ad(){
                  $.ajax({
                        url:urlN+"/api/Demand/MapGetRightFour",
                        dataType:"json",
                        type:"get",
                        data:data_param,
                        xhrFields:{
            ...
            var BeginDate = "",
             ...
             function get_data_dem(){
                  $.ajax({
                        url:urlN+"/api/Demand/MapGetRightFive",
                        dataType:"json",
                        type:"get",
                        data:dem_param,
                        xhrFields:{
            ...
            $("#down_data").off("click").on("click",function(){
                        ...
                        //请求数据
                        $.ajax({
                              url:urlN+"/api/Demand/MapExportToExcel",
                              type:"post",
                              xhrFields:{
                                    withCredentials:true
                              },
                              ...
                              success: function(data){
                                    if(data.Status == 0){
                                          //下载报表
                                          // $("#down_data a").attr("href","http://bop1.chitunion.com" + data.Result);
                                          window.location = urlN + data.Result
                                    }
