...
using XYAuto.BUOC.BOP2017.Entities;
    ...
    public class Util
    {
                    ...
                    using (SqlTransaction transaction = connection.BeginTransaction())
                    {
                                   ...
                                   transaction))
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                //Loger.Log4Net.Error(ex.Message, ex);
        ...
        public static T BindQuery<T>(HttpContext context)
        {
            ...
            foreach (var property in entityType.GetProperties())
            {
                ...
                {
                    //Loger.Log4Net.Info("转换model实体类（" + typeof(T).Name + "）属性（" + property.Name + "）传值（" + (value1 == null ? "null" : value1.ToLower()) + "），忽略");
                }
            }
            return model;
        }
        ...
        public static byte[] FileToBinary(string path)
        {
            ...
            {
                //Loger.Log4Net.Error("读取文件 异常：", ex);
                return buffer;
            }
            finally
            {
        ...
        public static void BinaryToFile(string path, byte[] buffer)
        {
            ...
            {
                //Loger.Log4Net.Error("写入文件 异常：", ex);
            }
            finally
            {
                if (stream != null)
        ...
        public static T HttpWebRequestCreateWithCallBack<T>(string httpUrl)
        {
            try
            {
                var request = WebRequest.Create(httpUrl) as HttpWebRequest;
                ...
                response.Close();
                request.Abort();
                var obj = JsonConvert.DeserializeObject<T>(str.Split('(')[1].TrimEnd(')'));
                return obj;
            }
            catch (Exception ex)
            {
        ...
        private static void ReadCallback(IAsyncResult asynchronousResult)
        {
            ...
            string str = string.Empty;
            StreamReader sr = new StreamReader(postStream, Encoding.UTF8);
            str = sr.ReadToEnd();
            postStream.Close();
        }
        ...
        public static string CleanToBmpImg(Image image, string SuffixName)
        {
            string imageUrl = string.Empty;
