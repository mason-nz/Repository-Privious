...
namespace XYAuto.BUOC.BOP2017.BLL.GDT
{
    ...
    /// 封装对智慧云接口的封装
    /// </summary>
    public class LogicByZhyProvider : Infrastruction.Verification.VerifyOperateBase
    {
        ...
        public ReturnValue DemandStatusNote(ToDemandNotes notes)
        {
            ...
            var dicParams = GetSignatureParams()
                ...
                .Insert("OrganizeAdsStatus", status).Insert("HandleTime", notes.HandleTime)
                .Insert("Reject", notes.Reject);

            var postData = $"OrganizeID={notes.OrganizeId}&OrganizeAdsID={notes.DemandBillNo}&OrganizeAdsStatus={status}" +
                           $"&HandleTime={notes.HandleTime}&Reject={notes.Reject}";
            ...
            var result = new DoPostApiLogClient(requestUrl, postData).GetPostResult<RespZhyBaseDto<dynamic>>
               (s => HttpClient.PostByFrom(requestUrl, postData), Loger.ZhyLogger.Info);
        ...
        private int GetOrganizeAdsStatus(DemandAuditStatusEnum status)
        {
            switch (status)
            {
                ...
                case DemandAuditStatusEnum.IsOver:
                    return 5;//已结束-->已完成
                case DemandAuditStatusEnum.Revoke:
                    return 6;//撤销-->撤销
