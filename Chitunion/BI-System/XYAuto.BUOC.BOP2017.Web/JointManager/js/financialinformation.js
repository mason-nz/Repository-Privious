$(function () {
    ...
    financialinformation.prototype={
        ...
        parameter:function (val,i) {
            if(val==1){
                ...
                var xqdanhao=$('#xqdanhao input').val();
                var createDate=$('#createDate').val();
                var createDate1=$('#createDate1').val();
                return {
                    RechargeNumber:czdanhao,
                    DemandBillNo:xqdanhao,
                    UserName:czyonghu,
                    StarDate:createDate,
                    EndDate:createDate1,
                    PageIndex:i,
                ...
                var xqdanhao=$('#xqdanhao input').val();
                var xqmingcheng=$('#xqmingcheng input').val();
                var createDate=$('#createDate').val();
                var createDate1=$('#createDate1').val();
                return {
                    DemandBillNo:xqdanhao,
                    DemandBillName:xqmingcheng,
                    StarDate:createDate,
                    EndDate:createDate1,
                    PageIndex:i,
                    pageSize:20
                ...
                var xqmingcheng=$('#xqmingcheng input').val();
                var czdanhao=$('#czdanhao input').val();
                var ycleixing=$('#ycleixing select option:selected').val();
                return {
                    DemandBillNo:xqdanhao,
                    DemandBillName:xqmingcheng,
                    RechargeNumber:czdanhao,
                    ExceptionType:ycleixing,
                    PageIndex:i,
                    pageSize:20
                }
            }
                ...
                $.openPopupLayer({
                    ...
                    success: function (data) {
                        ...
                        $('#submitMessage1').off('click').on('click',function () {
                            ...
                            setAjax({
                                url:urlN+'/api/ZhyInfo/BingdingTradeRelation',
                                type:'post',
                                data: {
                                    TradeType:84001,
    ...
    $('.tab_menu li').off('click').on('click', function () {
        ...
        $('#xqmingcheng input').val('');//需求名称
        $('#xqdanhao input').val('');//需求单号
        ...
        $('#ycleixing select option').eq(0).prop('selected',true);//异常类型
        $('#createDate').val('');//提交时间
        ...
        switch(val){
            case 1:
                url=urlN+'/api/ZhyInfo/SelectGdtFlowDetail';
                $('#yonghuming').show();//用户名
                $('#xqdanhao').hide();//需求单号
                $('#xqmingcheng').hide()//需求名称
                $('#czdanhao').hide();//充值单号
                ...
                break;
            case 2:
                url=urlN+'/api/ZhyInfo/SelectGdtDateSummaryInfo';
                $('#yonghuming').hide();//用户名
                $('#xqdanhao').hide();//需求单号
                $('#xqmingcheng').hide()//需求名称
                $('#czdanhao').hide();//充值单号
                ...
                break;
            case 3:
                url=urlN+'/api/ZhyInfo/SelectDemandBillRecharge';
                $('#yonghuming').hide();//用户名
                $('#xqdanhao').show();//需求单号
                $('#xqmingcheng').hide()//需求名称
                $('#czdanhao').show();//充值单号
                ...
                break;
            case 4:
                url=urlN+'/api/ZhyInfo/SelectDemandSpendList';
                $('#yonghuming').hide();//用户名
                $('#xqdanhao').show();//需求单号
                $('#xqmingcheng').show()//需求名称
                $('#czdanhao').hide();//充值单号
                $('#gdtdanhao').hide();//GDT单号
                $('#czyonghu').hide();//充值用户
                $('#zhuangtai1').hide();//状态-充值对账
                ...
                break;    
            case 5:
                url=urlN+'/api/ZhyInfo/SelectBackAmountInfo';
                $('#yonghuming').show();//用户名
                $('#xqdanhao').show();//需求单号
                $('#xqmingcheng').hide()//需求名称
                $('#czdanhao').show();//充值单号
                ...
                break;
            case 6:
                url=urlN+'/api/ZhyInfo/SelectExceptionDataList';
                $('#yonghuming').hide();//用户名
                $('#xqdanhao').show();//需求单号
                $('#xqmingcheng').show()//需求名称
                $('#czdanhao').show();//充值单号
