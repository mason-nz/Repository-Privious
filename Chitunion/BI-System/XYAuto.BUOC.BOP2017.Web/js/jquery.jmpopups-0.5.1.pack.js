...
(function ($) {
    ...
    function setPopupLayersPosition(popupElement, animate) {
        if (popupElement) {
            ...
            if (popupElement.height() < $(window).height()) {
                var topPosition = $(document).scrollTop() + ($(window).height() - popupElement.height()) / 2;
            } else {
                var topPosition = $(document).scrollTop() + 5;
            }
            var positions = {
            ...
            for (var i = 0; i < openedPopups.length; i++) {
                setPopupLayersPosition($("#popupLayer_" + openedPopups[i].name), animate);
            }
        }
    }
    ...
    $(window).resize(function () {
        //setScreenLockerSize();
        setPopupLayersPosition(null, true);
    }).scroll(function () {
        for (var i = 0; i < openedPopups.length; i++) {
            var popupElement = $("#popupLayer_" + openedPopups[i].name);
            if (popupElement) {
                if (popupElement.height() < $(window).height()) {
    ...
    */
    $.jPopMsgLayer = function (msg, afterCloseCallBack, second) {
        ...
        //var btnPopup_ok = '<input type="button" value=" 确定 " onclick="javascript:$.closePopupLayer(\'PopMsgLayer\');" style="width: 50px; height: 22px;"/>';
        ...
        //            afterClose: afterCloseCallBack
        //        });
        setTimeout(function () { $.closePopupLayer('PopMsgLayer'); }, second != undefined ? second * 1000 : 2000);
    }
    //重新设定弹出层的位置。主要为了解决层中加载数据变化后导致层不在中间显示的问题，此方法需要在查询完数据后调用
    $.resetPopupLayersPosition = function (popupElementName, animate) {
        var popupElement = $("#popupLayer_" + popupElementName);
        if (popupElement) {
            if (popupElement.width() < $(window).width()) {
                var leftPosition = (document.documentElement.offsetWidth - popupElement.width()) / 2;
            } else {
