$(function(){
  ...
  setAjax({
      ...
      $('.blacklist').append(ejs.render($('#hits').html(),{data:data.Result.listDetail}));
      ...
      $(".cancel_favorites").on("click",function(){
                  console.log($(this));
                  var MediaID = $(this).find("input").eq(0).val();
                  var MediaType = $(this).find(".MediaType").val();
                  console.log(MediaID);
                  $(this).parent().remove();
                  //取消收藏操作
                  setAjax({
            ...
            $('.blacklist').html('<img src="/images/no_data.png" style="display:block;margin:20px auto;">');
          }
        })
    }
})
