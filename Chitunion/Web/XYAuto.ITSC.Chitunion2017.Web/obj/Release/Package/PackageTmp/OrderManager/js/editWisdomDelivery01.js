...
* Written by:     wangcan
* function:       智投编辑
* Created Date:   2017-08-16 新增页面
* Modified Date:  
*/
$(function () {

    var orderID = GetRequest().orderID;
    //checkedInfo是为了页面显示存储数据，selectInfo是为了调后台接口存储数据
	var checkedInfo = [],selectInfo = {},config = {};
    var WisdomDedivery = {
        initData : function () {//调查看接口初始化数据
        	var _this = this;
        	_this.displayCarInfo();
        	var url = 'http://www.chitunion.com/api/ADOrderInfo/IntelligenceADOrderInfoQuery?v=1_1';//根据项目号查看智投项目
            	// url = 'json/info.json';
            setAjax({
                url : url,
                type : 'get',
                ...
                if(data.Status == 0){
                	config = data.Result.AreaInfos;
                    $('.orderInfo').html(ejs.render($('#orderInfo').html(),{list:Result.AreaInfos}));
                    //所选城市信息渲染
					selectInfo = {
						AreaInfo:[],
						MasterID:Result.ADOrderInfo.MasterID,
						BrandID:Result.ADOrderInfo.BrandID,
						SerialID:Result.ADOrderInfo.SerialID,
						MasterBrand:Result.ADOrderInfo.MasterName,
						CarBrand:Result.ADOrderInfo.BrandName,
						CarSerial:Result.ADOrderInfo.SerialName,
						BudgetTotal:Result.ADOrderInfo.BudgetTotal,
						JKEntrance:Result.ADOrderInfo.JKEntrance,
						LaunchTime:Result.ADOrderInfo.LaunchTime,
						OrderRemark:Result.ADOrderInfo.OrderRemark
					}
					Result.AreaInfos.forEach(function(item){
						var singleInfo = {
							CityID:item.CityID,
							...
							OriginContain:item.OriginContain
						}
						checkedInfo.push(singleInfo);
                    ...
                    //推广目的
                    if(Result.ADOrderInfo.OrderRemark.indexOf('60001') != -1){
                    	$('.promotionPurpose').find('input:eq(0)').attr('checked',true);
                    }
                    if(Result.ADOrderInfo.OrderRemark.indexOf('60002') != -1){
                    	$('.promotionPurpose').find('input:eq(1)').attr('checked',true);
                    }
                    if(Result.ADOrderInfo.OrderRemark.indexOf('60003') != -1){
                    	$('.promotionPurpose').find('input:eq(2)').attr('checked',true);
                    }
                    if(Result.ADOrderInfo.OrderRemark.indexOf('60004') != -1){
                    	$('.promotionPurpose').find('input:eq(3)').attr('checked',true);
                    }
                    //车型品牌渲染
		            $('.brand').find('option').each(function(i,item){
						if($(item).attr('MasterID') == Result.ADOrderInfo.MasterID){
							$(item).attr('selected',true);
							$('.brand').change();
						}
					});
					$('.series').find('option').each(function(i,item){
						if($(item).attr('brandid') == Result.ADOrderInfo.BrandID){
							$(item).attr('selected',true);
							$('.series').change();
						}
					});
					$('.Models').find('option').each(function(i,item){
						if($(item).attr('carserialid') == Result.ADOrderInfo.SerialID){
							$(item).attr('selected',true);
						}
					});
                	//预计投放日期
                	$('#deleverDay').val(Result.ADOrderInfo.LaunchTime.substr(0,10));
		            
				    //当预计投放日期小于今天后的20天时，提示
				    var curTime = new Date();
				    var tommorow20 = new Date((+curTime)+20*24*3600*1000).format('yyyy-MM-dd');
				    // console.log(tommorow20);
                	if(Result.ADOrderInfo.LaunchTime.substr(0,10) < tommorow20){
                		$('.deleverDay').find('.hidden_tip').html('<img src="../images/icon21.png"> 预计投放日期需为20天以后的日期，请修改！').show();
                	}
                	//集客入口
                	// console.log($('.exit').find('input:eq(0)'))
                	if(Result.ADOrderInfo.JKEntrance){
                		$('.exit').find('input:eq(0)').attr('checked','checked');
                	}else{
                		$('.exit').find('input:eq(1)').attr('checked','checked');
                	}
		            _this.getAllCount();
		            _this.operation();
		            _this.allCheck();
        ...
        displayCarInfo : function(){//车型品牌展示
        	$.ajax({
        ...
        operation : function(){//操作
			...
			$('#intelligentDelivery').off('click').on('click',function(){
				...
				if( !curSelectInfo.LaunchTime){
					$('.deleverDay').find('.hidden_tip').html('<img src="../images/icon21.png">请选择预计投放日期').show();
				...
				if($('.deleverDay').find('.hidden_tip:visible').length == 1){
					deleverError.push('投放日期有问题');
				}
				if(deleverError.length == 0){
		            ...
		            $('.orderInfo').html('');
					_self.getAllCount();
		            getInitalData(checkedInfo,selectInfo);
				}
			})
			//确认投放
			$('#nextBtn').off('click').on('click',function(){
				if($('.orderInfo .mediaInfo[isSelected=1]:visible').length > 0){
					...
					// console.log(upData,"这是最终数据");
					if(upData.ADOrderInfo.LaunchTime.substr(0,10) < new Date().format('yyyy-MM-dd')){
		            ...
		            setAjax({
						...
						if(data.Status == 0){
							window.location = '/OrderManager/shopcartForWisdomDelivery02.html?orderID='+GetRequest().orderID;
						}else{
			...
			//获取智投返回数据
			function getInitalData(checkedInfo,selectInfo){
		        setAjax({
        ...
        allCheck:function(){//全选、单选、原创参考价选择
        	var _self = this;
        ...
        getAllCount : function() {// 计算广告位个数及价格
            //媒体数目
        ...
        getAllInfo:function(selectInfo){//获取页面上信息
        	var upData = {
			   "optType" : 2,
			   "ADOrderInfo" : {
