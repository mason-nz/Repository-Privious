    ...
    var DetailOfOrder = {
        Init : function () {
            ...
            setAjax({
                ...
                if(data.Status == 0){
                    ...
                    config.ChannelItem.forEach(function(item){
                        item.Channelinfo.forEach(function(each){
                            var obj = {
                                ChannelID : each.ChannelID,
                                PayType : each.PayType,
                                PayTypeName : each.PayTypeName,
                                PayMode : each.PayMode,
                                PayModeName : each.PayModeName,
                                UnitCost : each.UnitCost,
                                ChannelType : each.ChannelType,
                                ChannelTypeName : each.ChannelTypeName,
                                PayType1 : each.PayType,
                                PayTypeName1 : each.PayTypeName,
                                PayMode1 : each.PayMode,
                                PayModeName1 : each.PayModeName,
                                UnitCost1 : each.UnitCost,
                                ChannelType1 : each.ChannelType,
                                ChannelTypeName1 : each.ChannelTypeName,
                                MediaTypeName : each.MediaTypeName,//媒体
                                MediaNumber : each.MediaNumber,//账号
                                MediaName : each.MediaName//名称
                            }
                            if(each.PayMode == -2){//免费 的时候给付费模式默认值
                                obj.PayMode = 72001;
                                obj.PayMode1 = 72001;
                            }
                            if(each.PayModeName == null){//免费
                                obj.PayModeName = 'CPM';
                                obj.PayModeName1 = 'CPM';
                            }
                            ChanelItemArr.push(obj);
                        })
                    })
                    config.ChanelItemArr = ChanelItemArr;
                    Item_Info_arr = ChanelItemArr;
                    $('.install_box').html(ejs.render($("#chanel-base").html(),{Result:config}));
                ...
                function SendToData (){
                    ...
                    for(var i = 0;i<Media_Arr.length;i++){
                        ...
                        var obj = {
                            ChannelID : 0,
                            PayType : PayType,
                            PayTypeName : PayType_text,
                            PayMode : PayMode,
                            PayModeName : PayMode_text,
                            UnitCost : UnitCost,
                            ChannelType : ChannelType,
                            ChannelTypeName : ChannelType_text,
                            PayType1 : PayType,
                            PayTypeName1 : PayType_text,
                            PayMode1 : PayMode,
                            PayModeName1 : PayMode_text,
                            UnitCost1 : UnitCost,
                            ChannelType1 : ChannelType,
                            ChannelTypeName1 : ChannelType_text,
                            MediaTypeName : MediaTypeName_text,//媒体
                            MediaNumber : num_name.split('|')[0],//账号
                            MediaName : num_name.split('|')[1]//名称
                        }
                        config.ChanelItemArr.push(obj);
                    }
        ...
        Operation : function(){//  编辑  更新  删除  取消
            ...
            $('.adposition_table table tbody .edit').off('click').on('click',function(){
                ...
                var ChannelTypeName = Tr.attr('ChannelTypeName');//渠道类型
                var PayTypeName = Tr.attr('PayTypeName');//费用类型
                var PayModeName = Tr.attr('PayModeName');//付费模式
                ...
                Tr.find('select[name=ChannelType]').on('change',function(){
                    ...
                    Tr.attr('ChannelTypeName1',ChannelType_text);
                    config.ChanelItemArr[idx].ChannelType = ChannelType;
                    config.ChanelItemArr[idx].ChannelType_text = ChannelType_text;
                })
                ...
                Tr.find('select[name=PayType]').on('change',function(){
                    ...
                    Tr.attr('PayTypeName1',PayType_text);
                    config.ChanelItemArr[idx].PayType = PayType;
                    config.ChanelItemArr[idx].PayType_text = PayType_text;
                    if(PayType_text == '收费'){
                        Tr.find('select[name=PayMode]').show();
                        Tr.find('input[name=UnitCost]').show();
                    }else{
                ...
                Tr.find('select[name=PayMode]').on('change',function(){
                    ...
                    Tr.attr('PayModeName1',PayMode_text);
                    config.ChanelItemArr[idx].PayMode = PayMode;
                    config.ChanelItemArr[idx].PayMode_text = PayMode_text;
                })
            ...
            $('.adposition_table table tbody .update').off('click').on('click',function(){
                ...
                var ChannelType_text1 = Tr.attr('ChannelTypeName1');
                var PayType_text1 = Tr.attr('PayTypeName1');
                var PayMode_text1 = Tr.attr('PayModeName1');
            ...
            $('.adposition_table table tbody .delete').off('click').on('click',function(){
                ...
                layer.confirm('确认要删除数据吗', {
                    ...
                    if(len > 1){
                        ...
                        config.ChanelItemArr.splice(idx,1);
                    }else{
                        that.parents('tr').remove();
                        config.ChanelItemArr.splice(idx,1);
                    }
