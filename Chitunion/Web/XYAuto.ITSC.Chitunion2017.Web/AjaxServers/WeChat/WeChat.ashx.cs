...
namespace XYAuto.ITSC.Chitunion2017.Web.AjaxServers.WeChat
{
    ...
    public class WeChat : IHttpHandler
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string Notice(PostModel postModel)
        {
            var logPath = HttpContext.Current.Server.MapPath(string.Format("~/App_Data/Open/{0}/", DateTime.Now.ToString("yyyy-MM-dd")));
            ...
            try
            {
                ...
                var messageHandler = new CustomThirdPartyMessageHandler(HttpContext.Current.Request.InputStream, postModel);//初始化
                ...
                return messageHandler.ResponseMessageText;
            }
            catch (Exception ex)
            {
                throw;
        ...
        /// <returns></returns>
        public string Callback(Senparc.Weixin.MP.Entities.Request.PostModel postModel)
        {
            ...
            try
            {
                ...
                if (checkPublish)
                {
                    messageHandler = new OpenCheckMessageHandler(HttpContext.Current.Request.InputStream, postModel, 10);
                }
                else
                {
                    messageHandler = new CustomMessageHandler(HttpContext.Current.Request.InputStream, postModel, maxRecordCount);
                }
            ...
            catch (Exception ex)
            {
                using (
                    TextWriter tw =
                        new StreamWriter(HttpContext.Current.Server.MapPath("~/App_Data/Open/Error_" + DateTime.Now.Ticks + ".txt")))
                {
