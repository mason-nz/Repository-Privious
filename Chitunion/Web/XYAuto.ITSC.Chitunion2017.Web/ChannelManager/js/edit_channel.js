$( function () {
  ...
  setAjax({
    ...
    //渲染页面
    $(".install_box").html(ejs.render($("#edit_channel").html(),{data:data.Result}));
    ...
    function verify_value() {
      ...
      if($("#type_one input[type=radio]:checked").val() == 56001){
        ...
        if($("#type_two input[type=radio]:checked").val() == 57002 && $("#type_four input").val() == ""){
          ...
          $("#amount").hide();
          RebateType2 = 57002;
          RebateValue = parseInt($("#amount_val").val());
          console.log(RebateValue);
        }
    ...
    CooperateBeginDate = data.Result.CooperateBeginDate.split(" ")[0];
    CooperateEndDate = data.Result.CooperateEndDate.split(" ")[0];
    //给开始时间跟结束时间绑定点击的事件
