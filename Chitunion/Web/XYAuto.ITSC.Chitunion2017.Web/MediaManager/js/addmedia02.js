$(function () {
    ...
    Addeditmedia.prototype = {
        ...
        uploadImg: function (id, img, imgerr, bigImg) {
            ...
            // function postTest() {
            //     // setAjax({
            //     //     url: 'http://www.chitunion.com/api/Authorize/GetMenuInfo',
            //     //     type: 'get'
            //     // }, null);
            //     $.ajax({
            //         type: "get",
        ...
        information: function (_this) {
            ...
            _this.ClickSubmit(_this)
            /*微信号、微信名称模糊查询*/
            $('#Microsignalfilling1').off('keyup').on('keyup', function () {
                var val = $.trim($(this).val());
                var cs_z = [];
                if (val != '') {
                    $.ajax({
                        url: 'http://www.chitunion.com/api/media/SearchMedia?v=1_1',
                        type: 'get',
                        data: {
                            Number: val ? val : '-'
                        },
                        ...
                        success: function (data) {
                            if (data.Status == 0) {
                                ...
                                if($('#Microsignalfilling1').attr('code')!=$('#Microsignalfilling1').val()){
                                    $('#wechatname1').val('');
                                    // 头像
                                    $('#headimgUploadFile-Headportrait').attr('src',' ')
                                    $('#headBigImg-Headportrait').attr('src',' ').parent('.bigImgshow').hide();
                                    $('#headUploadify-Headportrait').parent('.set').show();
                                    $('#headimgUploadFile-Headportrait').unbind('mouseover').unbind('mouseout');
                                    // 二维码
                                    $('#headimgUploadFile-QRcode').attr('src',' ')
                                    $('#headBigImg-QRcode').attr('src',' ').parent('.bigImgshow').hide();
                                    $('#headUploadify-QRcode').parent('.set').show();
                                    $('#headimgUploadFile-QRcode').unbind('mouseover').unbind('mouseout');
                                    // 粉丝数
                                    $('#Fans').val('');
                                    // 常见分类
                                    $('#ification').html('');
                                    ...
                                    // 覆盖地区
                                    $('#fgdq_1 option').eq(0).prop('selected',true);
                                    $('#fgdq_2').html('<option value="-2">城市</option>');
                                    // 性别比例
                                    $('#male').val('');
                                    $('#female').val('');
                                    // 媒体级别
                                    $('#mediaLevel input').eq(0).prop('checked',false);
                                    $('#mediaLevel input').eq(1).prop('checked',false);
                                    // 微信认证
                                    $('#weChatConfirm input').eq(0).prop('checked',false);
                                    $('#weChatConfirm input').eq(1).prop('checked',false);
                                    //下单备注
                                    $('#xiadanbeizhu1').show().find('input').prop('checked',false);
                                    $('#xiadanbeizhu2').hide().find('input').prop('checked',false);
                                    // 描述签名
                                    $('.sign').val('');
                                    // 粉丝分布区域
                                    $('#Renfansdistri').html('');
                                }
                                $('#Microsignalfilling1').autocomplete({
                                    source: availableArr,
                                    select: function (event, ui) {
                                        $('#Microsignalfilling1').attr('code',ui.item.value)
                                        console.log(availableArr.indexOf(ui.item.value));
                                        if (availableArr.indexOf(ui.item.value) != -1) {
                                            candata.operateType = 1;
                                            candata.Version = 118;
                                            candata.BaseMediaId = data.Result[availableArr.indexOf(ui.item.value)].BaseMediaId;
                                            candata.MediaId = data.Result[availableArr.indexOf(ui.item.value)].MediaId;
                                            _this.myAjax({
                                                ...
                                                if (operatetype == 1) {
                                                        ...
                                                        for (var i = 0; i < data.Result.CommonlyClass.length; i++) {
                                                            for (var j = 0; j < dat.Result.length; j++) {
                                                                if (data.Result.CommonlyClass[i] == dat.Result[j].DictId) {
                                                                    arr.push({
                                                                        name: dat.Result[j].DictName,
                                                                        id: dat.Result[j].DictId,
                                                                        sub: j
                                                                    })
                                                                }
                                                            }
                                                    ...
                                                    if (data.Result.Number != '') {
                                                        // $('#Microsignalfilling1').val(data.Result.Number)
                                                        // .prop('disabled', true)
                                                    }
                                                    // 微信名称
                                                    if (data.Result.Name != '') {
                                                    ...
                                                    // 覆盖地区
                                                    // 省
                                                    console.log(data.Result.AreaMedia.length,'zheng');
                                                    if (data.Result.AreaMedia.length > 0) {
                                                        console.log(data.Result.AreaMedia[0].ProvinceId);
                                                    ...
                                                    // wxid
                                                    $('#Microsignalfill').attr('i', data.Result.WxId);
                                                    // 媒体级别
                                                    if ((typeof (data.Result.LevelType - 0)) == "number") {
                                                        $('#mediaLevel input').each(function () {
                                                            console.log($(this).val(), data.Result.LevelType);
                                                            console.log($(this).val() - 0 == data.Result.LevelType);
                                                            if ($(this).val() - 0 == data.Result.LevelType) {
                                                                console.log($(this));
                                                    ...
                                                    // 二维码
                                                    if (data.Result.TwoCodeURL ? data.Result.TwoCodeURL : '' != '') {
                                                        $('#headimgUploadFile-QRcode').attr('src', data.Result.TwoCodeURL).siblings('.set').hide();
                                                        $('#headBigImg-QRcode').attr('src', data.Result.TwoCodeURL);
                                                        $('#headimgUploadFile-QRcode').off('mouseover').on('mouseover', function () {
                                                            $('#headBigImg-QRcode').parent().show();
                                                        }).off('mouseout').on('mouseout', function () {
            ...
            /*添加文本框聚焦事件，使其默认请求一次*/
            $('#Microsignalfilling1').off('focus').on('focus', function () {
        ...
        ClickSubmit: function (_this, data) {
            ...
            $('#Submit1').off('click').on('click', function () {
                ...
                if (_this.GetRequest().WxID == 0) {
                    ...
                    // 判断头像
                    console.log($('#headimgUploadFile-Headportrait').attr('src'));
                    if ($('#headimgUploadFile-Headportrait').attr('src') == '') {
                        arr.push('请上传头像')
                        $('#headimgUploadFile-Headportrait2').html('<span style="color: red"> 请上传头像</span>')
                    }
        ...
        Getparameters: function () {
            ...
            if (operatetype == 1) {
                obj.WeiXin.MediaId = $('#Microsignalfill').attr('i') - 0;
            }
            console.log(obj);
            return obj
        }
