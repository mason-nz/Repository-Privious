    ...
    public class Check : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (flag && lu != null && (lu.Category == 29001 || lu.Category == 29002))
            {
                ...
                weixinUser = ITSC.Chitunion2017.Dal.WeChat.WeiXinUser.Instance.GetUserInfoByUserId(lu.UserID, (UserCategoryEnum)lu.Category);
            ...
            if (flag)
            {
                ...
                context.Response.Write("document.write(\"<sc\"+\"ript>var CTLogin={ UserID:" + lu.UserID +
                    ",IsLogin:" + flag.ToString().ToLower() +
                    ",UserName:'" + lu.UserName + "',Mobile:'" + lu.Mobile +
                    "',TypeID:" + lu.Type + ",RoleIDs:'" + lu.RoleIDs +
                    "',Category:" + lu.Category + ",BUTIDs:'" + lu.BUTIDs +
                    "',HasMsgCount:" + hasMsgCount + ",Source:" + lu.Source + ",RegisterType:" + (lu.RegisterType > 0 ? lu.RegisterType : -1) +
                    ", WX_Nickname: '" + (weixinUser != null ? weixinUser.nickname : "") + "', WX_HeadimgUrl: '" + (weixinUser != null ? weixinUser.headimgurl : "") + "'};</scr\"+\"ipt>\");");
            }
            else
            {
                if (RequestNotRedirectURL)//未登录后，是否要跳转页面
                {
                    context.Response.Write("document.write(\"<sc\"+\"ript>var CTLogin={ UserID:-1,IsLogin:false,RealName:'',UserName:'',Mobile:'',TypeID:-1,RoleIDs:'',Category:-1,BUTIDs:'',Source:-1,RegisterType:-1,WX_Nickname:'',WX_HeadimgUrl:''};</scr\"+\"ipt>\");");
                }
                else
                {
