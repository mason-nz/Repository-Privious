...
{
    public class ImageCheckApi
    {
        public static readonly ImageCheckApi Instance = new ImageCheckApi();
        ...
        private string DunWangyi_ImgBusinessID = ConfigurationManager.AppSettings["DunWangyi_ImgBusinessID"];
        private string DunWangyi_ImgBusinessURL = ConfigurationManager.AppSettings["DunWangyi_ImgBusinessURL"];
        ...
        /// <returns></returns>
        public ImgResult Check(string articleID, string imgUrl, string articleUrl)
        {
            try
            {
                ...
                parameters.Add("businessId", DunWangyi_ImgBusinessID);
                ...
                image1.Add("name", string.IsNullOrWhiteSpace(articleUrl) ? imgUrl : articleUrl);
                image1.Add("type", 1);
                image1.Add("data", imgUrl);
                jarray.Add(image1);
                ...
                String result = Utils.doPost(client, DunWangyi_ImgBusinessURL, parameters, DunWangyi_InterfaceTimeOut);
                if (result != null)
                {
                    sw.Stop();
                    BLL.Loger.Log4Net.Info($"接口[{DunWangyi_ImgBusinessURL}]，调用耗时：{sw.ElapsedMilliseconds},返回数据为：{result}");
                    ImgResult ir = JsonConvert.DeserializeObject<ImgResult>(result);
                    return ir;
                }
                return null;
            }
            ...
            {
                BLL.Loger.Log4Net.Error($"调用网易云接口（图片检测）异常：参数:articleID={articleID},imgUrl={imgUrl}", ex);
                return null;
            }
        }
    }
