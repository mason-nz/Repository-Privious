...
namespace XYAuto.ITSC.Chitunion2017.WebService.Common
{
    public class HttpHelper
    {
        ...
        #region 
        public static T HttpWebRequestCreate<T>(string httpUrl, string postData, RequsetType RequsetType, int requestContentType = (int)RequestContentType.JSON)
        {
            try
            {
                var request = WebRequest.Create(httpUrl) as HttpWebRequest;
                request.ContentType = GetEnumOptText(typeof(RequestContentType), requestContentType);
                request.Method = RequsetType.ToString();
                if (RequsetType.ToString() == "POST")
                {
                    byte[] replybyte = Encoding.UTF8.GetBytes(postData);
                    request.ContentLength = replybyte.Length;
                    Stream newStream = request.GetRequestStream();
                    newStream.Write(replybyte, 0, replybyte.Length);
                    newStream.Close();
                }

                HttpWebResponse AddVehiclResponse = (HttpWebResponse)request.GetResponse();
