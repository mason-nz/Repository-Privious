...
namespace XYAuto.ITSC.Chitunion2017.BLL
{
    public class Util
    {
        ...
        //分支合并后，需要自行修改逻辑
        //详情页域名配置
        protected static string DominArticle = XYAuto.Utils.Config.ConfigurationUtil.GetAppSettingValue("DominArticle", false);
        /// <summary>
        /// 从App端，分享文章详情页面到微信端的域名列表
        /// </summary>
        protected static string DominArticleShare = XYAuto.Utils.Config.ConfigurationUtil.GetAppSettingValue("DominArticleShare", false);
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetIP(string FunctionName)
        {
            try
            {
                Loger.Log4Net.Info($"{FunctionName}获取IP:");
                //如果客户端使用了代理服务器，则利用HTTP_X_FORWARDED_FOR找到客户端IP地址
                string result = HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_FOR"];
                if (!string.IsNullOrEmpty(result))
                {
                    Loger.Log4Net.Info($"获取当前客户端ID：HTTP_X_FORWARDED_FOR={result}");
                    result = result.ToString().Split(',')[0].Trim();
                }
                Loger.Log4Net.Info($"获取当前客户端ID：REMOTE_ADDR={HttpContext.Current.Request.ServerVariables["REMOTE_ADDR"]}");
                Loger.Log4Net.Info($"获取当前客户端ID：UserHostAddress={HttpContext.Current.Request.UserHostAddress}");
                if (string.IsNullOrEmpty(result))
                {
            ...
            {
                Loger.Log4Net.Error($"{FunctionName}获取IP出错:", ex);
            }
            return "0.0.0.0";
        }
        ...
        public static void DrawStringByImage(string text, string fileOriginPath, string fileGenPath)
        {
            ...
            {
                BLL.Loger.Log4Net.Error("在图片上绘制文本出错，DrawStringByImage方法", ex);
            }
            finally
            {
                try
        ...
        public static T HttpWebRequestCreate<T>(string httpUrl)
        {
            ...
            {
                BLL.Loger.Log4Net.Error("抓取错误", ex);
        ...
        public static string ConvertSINAShortUrl(string url)
        {
            ...
            {
                BLL.Loger.Log4Net.Error("根据长url转换短url方法报错，长url为：" + url, ex);
                return "";
            }
        }
        ...
        /// <returns>返回的内容需要自行添加前缀，返回前缀（http://192.168.3.71/）+返回内容（/group1/M00/08/EE/oYYBAFpB3VSAHUXfAABpdUeMYDM253.jpg）</returns>
        public static string CleanImg(Image img)
        {
            string imageUrl = string.Empty;
        ...
        public static void AddWebCacheByMobile(string mobile, string content)
        {
            try
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"AddWebCacheByMobile begin 手机号:{mobile}，content:{content}");
                //System.Web.Caching.Cache c = new System.Web.Caching.Cache();
                var c = HttpRuntime.Cache;
                DateTime dt = DateTime.Now.AddMinutes(5);
                Dictionary<string, object> dict = (Dictionary<string, object>)c.Get(mobile);
                if (dict == null)
                {
                ...
                {
                    TimeSpan ts = DateTime.Parse(dict["EndTime"].ToString()) - DateTime.Now;
                    dt = DateTime.Parse(dict["EndTime"].ToString()).AddSeconds(-ts.TotalSeconds);
                    dict["StartTime"] = dt;
                    dict["Time"] = ((int)dict["Time"]) + 1;
                }
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"AddWebCacheByMobile 字典:{JsonConvert.SerializeObject(dict)}");
                c.Insert(mobile, dict, null, (DateTime)dict["EndTime"], System.Web.Caching.Cache.NoSlidingExpiration);


                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"AddWebCacheByMobile end 手机号:{mobile},get:{JsonConvert.SerializeObject(c.Get(mobile))}");
            }
            catch (Exception ex)
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"AddWebCacheByMobile出错 手机号:{mobile}");
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"AddWebCacheByMobile出错:{ex.Message},StackTrace:{ex.StackTrace}");
            }
        }
        ...
        public static string GetMobileCheckCodeByCache(string mobile)
        {
            try
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeByCache begin mobile:{mobile}");
                //System.Web.Caching.Cache c = new System.Web.Caching.Cache();
                var c = HttpRuntime.Cache;
                Dictionary<string, object> dict = (Dictionary<string, object>)c.Get(mobile);
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeByCache 字典:{JsonConvert.SerializeObject(dict)}");
                if (dict == null)
                {
                    return null;
                }
                int times = (int)dict["Time"];
                string content = dict["SmsContent"].ToString();
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeByCache end code:{content},Times={times}");
                //AddWebCacheByMobile(mobile, content);
                return content;
            }
            catch (Exception ex)
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeByCache出错 手机号:{mobile}");
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeByCache出错:{ex.Message},StackTrace:{ex.StackTrace}");
                return null;
            }
        }
        ...
        public static int GetMobileCheckCodeTimesByCache(string mobile)
        {
            try
            {
                ...
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeTimesByCache begin mobile:{mobile}");
                //System.Web.Caching.Cache c = new System.Web.Caching.Cache();
                var c = HttpRuntime.Cache;
                Dictionary<string, object> dict = (Dictionary<string, object>)c.Get(mobile);
                if (dict == null)
                ...
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetMobileCheckCodeTimesByCache end code:{content},Times={times}");
                //AddWebCacheByMobile(mobile, content);
                return times;
            }
            catch (Exception ex)
            {
                ...
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Error($"GetMobileCheckCodeTimesByCache出错 手机号:{mobile}", ex);
                return -1;
            }
        }
        ...
        public static string GetCodeByCache(string cacheKey)
        {
            try
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetCodeByCache begin cacheKey:{cacheKey}");
                //System.Web.Caching.Cache c = new System.Web.Caching.Cache();
                var c = HttpRuntime.Cache;
                string code = (string)c.Get(cacheKey);
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetCodeByCache end code:{code}");
                return code;
            }
            catch (Exception ex)
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetCodeByCache出错 key:{cacheKey}");
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"GetCodeByCache出错:{ex.Message},StackTrace:{ex.StackTrace ?? string.Empty}");
                return null;
            }
        }
        ...
        public static void InsertCache(string cacheKey, string content, int minutes)
        {
            try
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"InsertCache begin cacheKey:{cacheKey}，content:{content}");
                //System.Web.Caching.Cache c = new System.Web.Caching.Cache();
                var c = HttpRuntime.Cache;
                c.Insert(cacheKey, content, null, DateTime.Now.AddMinutes(minutes), TimeSpan.Zero);
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"InsertCache end cacheKey:{cacheKey},get:{(string)c.Get(cacheKey)}");
            }
            catch (Exception ex)
            {
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"InsertCache出错 cacheKey:{cacheKey}");
                ITSC.Chitunion2017.BLL.Loger.Log4Net.Info($"InsertCache出错:{ex.Message},StackTrace:{ex.StackTrace ?? string.Empty}");
            }
        }
        /// <summary>
        /// 检测串值是否为合法的格式
        ...
        /// <param name="MaxSize">最大存储大小（KB）</param>
        /// <returns></returns>
        public static string ValidateImgSize(string FileUrl, decimal MaxSize)
        {
            try
            {
                ...
                if (mUri == null)
                {
                    Loger.Log4Net.Error("文件地址：" + FileUrl + "不存在");
                    return "文件不存在";
                }
                ...
                decimal aSize = (mResponse.ContentLength / 1024);
                //Image mImage = Image.FromStream(mStream);
                //int imgWith = mImage.Width;
                //int imgHeight = mImage.Height;
                mStream.Close();
                if (aSize > MaxSize)
                {
                    return $"文件不得超过{MaxSize / 1024}M";
                }
            }
            catch (Exception ex)
            {
                Loger.Log4Net.Error("文件地址：" + FileUrl + "不存在错误:" + ex.Message);
                return "文件不存在";
            }
            return "";
        }
        ...
        public static string WriteAppSettings(string keyName, string keyValue)
        {
            try
            {
                //获取Configuration对象
        ...
        public static string GetQueryString(string name, string url)
        {
            ...
            {
                BLL.Loger.Log4Net.Error("获取url字符串参数，返回参数值字符串[GetQueryString]出错", ex);
                return string.Empty;
            }
        }
