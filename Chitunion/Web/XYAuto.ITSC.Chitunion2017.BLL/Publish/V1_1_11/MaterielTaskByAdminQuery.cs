...
{
    public class MaterielTaskByAdminQuery
        : PublishInfoQueryClient<RequestTaskSchedulerDto, TaskSchedulerDto>
    {
        public MaterielTaskByAdminQuery(ConfigEntity configEntity) : base(configEntity)
        {
        }
        ...
        protected override PublishQuery<TaskSchedulerDto> GetQueryParams()
        {
            ...
            sbSql.AppendFormat(@"
                        ...
                        FROM    NLP2017.dbo.TR_GroupInfo AS g
                                JOIN NLP2017.dbo.TR_ArticleInfo AS a ON g.GroupID = a.GroupID
                                JOIN BaseData2017.dbo.ArticleInfo AS ai ON ai.RecID = a.ArticleID
                                ...
                                LEFT JOIN dbo.TaskScheduler_User AS TSU WITH(NOLOCK) ON TSU.GroupId = g.GroupID
            ...
            {
                //没有分配，即分配表为null
                baseSqlWhere.AppendFormat(@" AND TSU.TaskStatus IS NULL ");
            }
