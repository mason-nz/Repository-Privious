...
{
    public class GdtLogicProvider : CurrentOperateBase
    {
        ...
        public ReturnValue DoPullAccountUserInfo(int accountId = -2)
        {
            ...
            var accounInfotList = ServiceHelper.Instance.GetAdvertiserInfo(new ReqReportDto()
            {
                AccountId = accountId
            });
            if (accounInfotList.List == null)
            {
                var msg = $"获取广告主信息为null，accountId:{accountId}等待下一次执行";
            ...
            accounInfotList.List.ForEach(item =>
            {
                ...
                {
                    Loger.GdtLogger.ErrorFormat($"{System.Environment.NewLine}DoPullAccountUserInfo入库失败：{JsonConvert.SerializeObject(item)}");
                }
            });
        ...
        public ReturnValue DoPullGetFundsInfo(int accountId)
        {
            ...
            if (excuteCount < 1)
            {
                Loger.GdtLogger.ErrorFormat($"{System.Environment.NewLine}DoPullGetFundsInfo入库失败：{JsonConvert.SerializeObject(list)}");
            }
        ...
        public ReturnValue DoPullGetReportHourly(ReqReportDto request)
        {
            ...
            var insertList = AutoMapper.Mapper.Map<List<RespReportDto>, List<Entities.GDT.GdtHourlyRrport>>(pageInfo.List);
            //第一页处理
         ...
         RespPageInfo<List<RespReportDto>> pageInfo, Entities.GDT.GdtHourlyRrport entityWhere)
        {
            ...
            return CreateSuccessMessage(retValue, "", "GetReportHourly  获取小时报表入库完成");
        }
