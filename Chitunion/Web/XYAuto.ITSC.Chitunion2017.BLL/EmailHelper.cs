...
namespace XYAuto.ITSC.Chitunion2017.BLL
{
    public class EmailHelper
    {
        ...
        public void SendWechatReportDataMail(string[] reciver, string attachPath)
        {
            string[] body = new string[] { "微信数据统计" };
            SendMailByTemplateAndAttach("mail630", body, "微信数据统计", reciver, null, attachPath);
        }
                ...
                try
                {
                    ...
                    if (copyto != null)
                    {
                        for (int i = 0; i < copyto.Length; i++)
                        {
                            ...
                            BLL.Loger.Log4Net.Info("发送邮件给谁:" + copyto[i]);
                        }
                    }
                    mail.Send();
        ...
        public Attachment AddAttachments(string attachmentsPath)
        {
            ...
            try
            {
                ...
                data = new Attachment(path[0], MediaTypeNames.Application.Octet);
                return data;
            }
            catch (Exception ex)
            {
                BLL.Loger.Log4Net.Info("获取附件路径发生异常：" + ex);
                return null;
            }
        }
    }
