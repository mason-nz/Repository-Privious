...
namespace XYAuto.ITSC.Chitunion2017.Dal.GDT
{
    ...
    public class GdtStatementsDetailed : DataBase
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Insert(XYAuto.ITSC.Chitunion2017.Entities.GDT.GdtStatementsDetailed entity)
        {
            ...
            Entities.GDT.GdtStatementsDetailed entityWhere, int pageIndex)
        {
            ...
            if (pageIndex == 1)
            {
                sql.AppendFormat(@"DELETE FROM DBO.GDT_StatementsDetailed
                                ...
                                AND DATEDIFF(dd,[Date],'{2}')=0", entityWhere.AccountId, entityWhere.FundType, entityWhere.Date.ToString("yyyy-MM-dd"));
            }
            sql.AppendFormat(@"
