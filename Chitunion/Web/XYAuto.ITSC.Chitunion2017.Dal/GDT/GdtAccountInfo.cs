...
namespace XYAuto.ITSC.Chitunion2017.Dal.GDT
{
    ...
    public class GdtAccountInfo : DataBase
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Insert(XYAuto.ITSC.Chitunion2017.Entities.GDT.GdtAccountInfo entity)
        {
        ...
        public void InsertUserRole(int userId, string roleId, string trueName, string contactPerson, int organizeId)
        {
            ...
            var usrRoleSql = string.Format(@"
                        IF NOT EXISTS(SELECT 1 FROM DBO.UserRole WITH(NOLOCK) WHERE UserID = {0} AND SysID = '{1}'
            ...
            var userDetailSql = string.Format(@"
                        ...
                        IF EXISTS(SELECT 1 FROM DBO.UserDetailInfo WITH(NOLOCK) WHERE UserID = {0})
                        BEGIN
	                        UPDATE DBO.UserDetailInfo SET TrueName = '{1}',Contact = '{2}' WHERE UserID = {0}
                            SET @tran_error = @tran_error + @@error;
                        END
                        ELSE
                        BEGIN
        ...
        {
            var sql = @"SELECT * FROM DBO.UserInfo WITH(NOLOCK) WHERE Mobile = @Mobile AND Status = 0";
        ...
        public Entities.GDT.UserInfo VerifyOfRole(string mobile, string roleId)
        {
            var sql = @"
                        ...
                        WHERE   EXISTS ( SELECT 1
                                         FROM   dbo.UserRole AS UR WITH ( NOLOCK )
        ...
        public int InsertByGdtServer(XYAuto.ITSC.Chitunion2017.Entities.GDT.GdtAccountInfo entity)
        {
            var sql = string.Format(@"
                        DELETE FROM DBO.[GDT_AccountInfo] WHERE AccountId = {0}
