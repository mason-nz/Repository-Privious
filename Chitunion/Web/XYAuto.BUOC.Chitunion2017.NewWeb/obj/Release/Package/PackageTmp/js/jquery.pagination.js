...
jQuery.fn.pagination = function (maxentries, opts) {
    opts = jQuery.extend({
        ...
        num_display_entries: 3, //页码共有多少组
        current_page: 1,
        num_edge_entries: 1, //显示几个临近的页码
        ...
        isShowTotalMsg: true, //是否要显示总数信息
        isShowSkipPage: false, //是否要显示跳转页面部分
        callback: function () { return false; }
    }, opts || {});
    jQuery.extend(opts, { current_page: opts.current_page - 1 }); //修正页面索引从1开始
    ...
    return this.each(function () {
        ...
        */
        function pageSelected(page_id, evt) {
            ...
            drawLinks();
            var continuePropagation = opts.callback(page_id + 1, panel); //修正页面索引从1开始
        ...
        */
        function drawLinks() {
            ...
            if (opts.isShowSkipPage && opts.isShowSkipPage == true) {
                /*插入一个文本框，用户输入并回车后进行跳转*/
                var pagetext = '<input id="pagevalue" size="1" value="' + (current_page + 1) + '"type="text">';
                var toPage = '<span><a id="search" href="' + opts.link_to + '">跳转</a></span>';
            ...
            if (opts.isShowTotalMsg && opts.isShowTotalMsg == true) {
                var totalMsg = '<span class="NormalText">共有' + maxentries + '条数据</span>';
                $(totalMsg).appendTo(panel);
            }
        }
