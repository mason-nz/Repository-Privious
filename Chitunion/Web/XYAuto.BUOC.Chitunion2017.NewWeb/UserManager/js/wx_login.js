...
var WXLoginHelper = window.WXLoginHelper = (function () {
    ...
    var cookieDomain = public_url && public_url.indexOf('://') > 0 ? '.' + public_url.substring(public_url.indexOf('://') + 3) : '.www.chitunion.com';  // public_url.indexOf('://') '.www1.chitunion.com';
    //解析参数
    ...
    var setCookie = function (name, value, expir) {
        ...
        cookie += ';domain=' + cookieDomain + ';path=/;';
    ...
    var Load_QRCode = function (categoryid) {
        ...
        $('div.main div.left div.two_rules div.we_code p[name="pwx_tag"]').text('打开微信扫一扫登录 赤兔联盟');
        var loginTypeype = -2;
        ...
        function settime(imgObj) {
            ...
            setAjax(setting, function (data) {
                if (data && data.Status == 0) {
                    ...
                    imgObj.siblings('p[name="pwx_tag"]').text('用户信息异常，请联系客服');
                    //imgObj.siblings('p.bottom').hide();
                    //imgObj.siblings('p[name="pwx_OK"]').show();
                }
            }, function (error) {
