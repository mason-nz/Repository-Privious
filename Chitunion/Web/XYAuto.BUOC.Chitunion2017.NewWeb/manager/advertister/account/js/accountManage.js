	...
	AccountManage.prototype = {
		...
		operate:function(){
		    ...
		    $('#modifyAlipay').off('click').on('click',function(){
		    	...
		    	$('#oldAlipayAccount,#newAlipayAccount,#sureNewAlipayAccount').val('');
		    	$('#submitInfo').show();
		    })
				...
				if(data.Status == 0){
					...
					if(AuthenticationInfo){
						...
						if(AuthenticationInfo.Status == 0){//未认证
							$authInfo.find('.before').removeClass('hide').siblings('.after').addClass('hide');
							$authInfo.find('.after .authRetrunInfo').addClass('hide');
						}else if(AuthenticationInfo.Status == 1){//待审核
							$authInfo.find('.after').removeClass('hide').siblings('.before').addClass('hide');
					...
					if(BasicInfo.Category == '29002'){//媒体主有提现账号
						if(BankAccountInfo && BankAccountInfo.AccountType == '96001'){//支付宝类型
							if(BankAccountInfo.AccountName){//存在支付宝
								$('.account').find('.after').removeClass('hide').end().find('.before').addClass('hide').end().find('.afterSubmit').addClass('hide');
								var curAccount = BankAccountInfo.AccountName,
									curAccountLen = curAccount.length,
									setAccount = curAccount;
								if(curAccountLen>4){
									if(curAccount % 2){
										setAccount = curAccount.substr(0,(curAccountLen-5)/2)+'****'+curAccount.substr((curAccountLen-5)/2+4)
									}else{
										setAccount = curAccount.substr(0,(curAccountLen-4)/2)+'****'+curAccount.substr((curAccountLen-4)/2+4)
									}
								}
								$('#alipayAccount_m').html(setAccount);
