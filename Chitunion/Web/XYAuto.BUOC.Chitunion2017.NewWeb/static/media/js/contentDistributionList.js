...
//                  console.log(data.Result.TotleCount+'+data.Result.TotleCount');
                    ...
                    if (CTLogin.Category == 29002) {
                        ...
                        if (flag){//分享已上线
                            layer.confirm('当日分享次数已达上限，是否无偿分享？', function(index){
                                _this.toAjaxData(that, _this, TaskType, TaskId);
                                layer.close(index);
                            });
                            return false
                        } else {
        ...
        //领取任务Receive回调
        receiveCallbackFn: function(data, that, _this, TaskType, TaskId){
            ...
            setTimeout(function () {
                if (data.Status == 0 || data.Status == 1004) {//0：未领取，1004：已领取
                    ...
                    if(TaskType == '192001'){
                        ...
                        str = _this.toGetShareModuleFenf(dataTitle, pasterUrl, dataOrderUrl);
                        ...
                        str = _this.toGetShareModuleTiep(titMain, titSub, dataDownloadImg);
                        that.parents('.task').find($('.exclusiveinfo')).css({'display': 'block'}).html(str);
                         //下载
                        var aBtn = that.parents('.task').find($(".download_img_a"));
                        if (_this.browserIsIe()) {
                            //是ie等,绑定事件
        ...
        toAjaxData: function(that, _this, TaskType, TaskId){
        	$.ajax({
                ...
                success: function (data) {
                    setTimeout(function(){
                    	_this.receiveCallbackFn(data, that, _this, TaskType, TaskId);
                    }, 40)
        ...
        //下载图片的函数
		downloadImg: function (TaskId, imgSrc) {
		    //iframe的src属性不为空,调用execCommand(),保存图片
		    if (imgSrc) {
                console.log(imgSrc+'+imgSrcimgSrcimgSrcimgSrc000000')
                // window.frames["IframeReportImg_"+TaskId].document.execCommand("SaveAs");
