...
namespace XYAuto.ITSC.Chitunion2017.GdtPullData.Provider
{
    ...
    internal class GdtAdsScheduler
    {
        ...
        public class DoPullGetAdGroupList : IJob
        {
            public void Execute()
            {
                BLL.Loger.GdtLogger.Info($" GdtAdsScheduler DoPullGetAdGroupList start..");
        ...
        public class DoPullGetAdCampaignsList : IJob
        {
            public void Execute()
            {
                BLL.Loger.GdtLogger.Info($" GdtAdsScheduler DoPullGetAdCampaignsList start..");
                var request = new ReqReportDto()
                {
                    AccountId = AppSettings.GdtAccountId,
                };
