...
namespace XYAuto.BUOC.ChiTuData2017.BLL.QueryPage
{
    /// <summary>
    /// Auth:李雄
    /// 刊例查询接口抽象类
        ...
        new()
    {
        ...
        public BaseResponseEntity<TResponse> GetQueryList(T query, string excel)
        {
            RequetQuery = query;
            try
            {
        ...
        protected BaseResponseEntity<TResponse> QueryOperate(T query, string excel)
        {
            queryParams = GetQueryParams();
            LogInfo(string.Format("{0} 查询条件信息:{1}",
                    System.Environment.NewLine, JsonConvert.SerializeObject(query)));
            queryParams.StrSql = queryParams.StrSql.Replace("YanFaFROM", " FROM ");
            var list = Dal.QueryPage.PublicPageQuery.Instance.QueryList<TResponse>(queryParams, "");
            return GetResult(list, queryParams);
        }
