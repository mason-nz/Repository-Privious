...
namespace XYAuto.BUOC.ChiTuData2017.PullStatisticsData.Scheduler.Registry
{
    ...
    public class ZhyPullDistributeDetailSchduler : IJob
    {
        ...
        public ZhyPullDistributeDetailSchduler()
        {
            ...
            {
                DateOffset = PullDataSettings.Instance.ExcuteDateOffsetDistributeDetail,//分发明细 偏移量（拉取明细-25 到指定的节点时间）
            });
        }
        ...
        public void Execute()
        {
            ...
            //发送邮件通知
            var queryDay = DateTime.Now.AddDays(PullDataSettings.Instance.ExcuteDateOffsetDistributeDetail).ToString("yyyy-MM-dd");
