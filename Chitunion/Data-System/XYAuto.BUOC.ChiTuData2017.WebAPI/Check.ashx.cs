...
namespace XYAuto.BUOC.ChiTuData2017.WebAPI
{
    ...
    public class Check : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            bool flag = ITSC.Chitunion2017.Common.UserInfo.IsLogin(out lu);
            if (flag)
            {
                if (!RequestNotCheckModule)//若请求时，传递参数NotCheckModule=true，则跳过权限验证逻辑
                {
                    pid = "";
                    moduleID = ITSC.Chitunion2017.Common.UserInfo.CheckUserRight(ref pid, true);  // .Common.EmployeeInfo.checkUserRight(ref pid, true);
                }
            ...
            if (flag)
            {
                int hasMsgCount = 0;//BLL.WeChatOperateMsg.Instance.GetCount_MsgNoRead();
            ...
            else
            {
                ...
                {
                    context.Response.Write("document.write(\"<sc\"+\"ript>window.location='" + XYAuto.Utils.Config.ConfigurationUtil.GetAppSettingValue("ExitAddress") + "/LoginExternal.aspx?gourl=' + encodeURIComponent(window.location + '');" + "</scr\"+\"ipt>\");");
                }
            }
        }
