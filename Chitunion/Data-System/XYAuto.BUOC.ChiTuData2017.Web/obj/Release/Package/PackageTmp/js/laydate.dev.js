...
; !function (win) {
    ...
    var config = {
        ...
        init: true,
        //issure: 1,
        isInitCheck: true,//是否初始化控件时，检测日期的有效性
        voidDateRange: [],//无效区间范围，如,[{'S':'2017-02-01','E':'2017-02-10'},{'S':'2017-03-01','E':'2017-10-20'}]
        selectedPoint: [],//已选择的时间点，如['2017-05-11','2017-05-21']
        isNeedConfirm: false,//是否需要确认后，关闭弹出日历控件层
        isShowHoliday: false,//是否展示节假日，默认为不展示；否则在日历控件中，替换日期内容为节假日文本
        showHolidayRequestURL: '/api/common/GetHolidaysInfo?jsonpcallback=?'//请求节假日后端api接口url，jsonp格式
    };

    var Dates = {}, doc = document, creat = 'createElement', byid = 'getElementById', tags = 'getElementsByTagName';
    var as = ['laydate_box', 'laydate_void', 'laydate_click', 'LayDateSkin', 'skins/', '/laydate.css', 'laydate_curClick'];
    ...
    Dates.run = function (options) {
        ...
        //渲染无效日期逻辑，Modify=masj,Date=2017-05-02
        Dates.RenderVoidDateRange();
        //渲染当前选择好的日期，内部函数
        Dates.RenderSelectedPoint();
    };

    //检测日期类型（Modify=masj，Date=2017-05-02）
    Dates.checkDate = function (val) {
    ...
    //渲染无效日期，内部函数（Modify=masj，Date=2017-05-02）
    Dates.RenderVoidDateRange = function () {
        if (Dates.options.voidDateRange && $.isArray(Dates.options.voidDateRange)) {
            var flag = false;
            if (Dates.options.voidDateRange.length > 0) {
                for (var i = 0; i < Dates.options.voidDateRange.length; i++) {
                    var startDate = Dates.options.voidDateRange[i].S;
                    var endDate = Dates.options.voidDateRange[i].E;
                    if (!(startDate && endDate &&
                        startDate.parseDate() <= endDate.parseDate())) {
                        flag = true;
                        break;
                    }
                }
                if (flag == false) {
                    for (var j = 0; j < Dates.options.voidDateRange.length; j++) {
                        var startDate = Dates.options.voidDateRange[j].S.parseDate();
                        var endDate = Dates.options.voidDateRange[j].E.parseDate();
                        while (startDate <= endDate) {
                            var year = startDate.Format('yyyy');
                            var month = startDate.Format('M');
                            var day = startDate.Format('d');
                            $('div.laydate_box > table.laydate_table > tbody > tr > td[y="' + year + '"][m="' + month + '"][d="' + day + '"]').addClass('laydate_void');
    ...
    //渲染当前选择好的日期，内部函数（Modify=masj，Date=2017-05-02）
    Dates.RenderSelectedPoint = function () {
        if (Dates.options.selectedPoint && $.isArray(Dates.options.selectedPoint)) {
            var flag = false;
            if (Dates.options.selectedPoint.length > 0) {
                for (var i = 0; i < Dates.options.selectedPoint.length; i++) {
                    var dateStr = Dates.options.selectedPoint[i];
                    if (!(dateStr && dateStr.isDate())) {
                        flag = true;
                        break;
                    }
                }
                if (flag == false) {
                    for (var j = 0; j < Dates.options.selectedPoint.length; j++) {
                        var dateObj = Dates.options.selectedPoint[j].parseDate();
                        var year = dateObj.Format('yyyy');
                        var month = dateObj.Format('M');
                        var day = dateObj.Format('d');
                        $('div.laydate_box > table.laydate_table > tbody > tr > td[y="' + year + '"][m="' + month + '"][d="' + day + '"]').addClass('laydate_click');
                    }
                }
            }
        }
    ...
    Dates.viewDate = function (Y, M, D, isNotRenderDayCSS) {
        ...
        Dates.each(as.tds, function (i, elem) {
            ...
            if (i < log.FDay) {
                ...
                if (i - log.FDay + 1 === log.ymd[2] && !isNotRenderDayCSS) {
            ...
            Dates.options.isShowHoliday && Dates.ShowHoliday(elem, YY, MM, DD);
    ...
    //从API接口中查出节假日,Modify=masj,Date=2017-06-09
    Dates.ShowHoliday = function (td, year, month, day) {
        if (Dates.options.isShowHoliday && Dates.options.isShowHoliday == true && config.showHolidayRequestURL && Dates.HolidaysData) {
            if (Dates.HolidaysData[year] && Dates.HolidaysData[year].length > 0) {
                $.each(Dates.HolidaysData[year], function (i, e) {
                    var startDate = e.StartDate.parseDate();
                    var endDate = e.EndDate.parseDate();
                    var holidayName = e.Name;
                    var currentDate = (year + '-' + month + '-' + day).parseDate();
                    if (currentDate >= startDate && currentDate <= endDate) {
                        td.innerHTML = "假";//holidayName.substring(0, 2);
                        $(td).attr('h', td.innerHTML);
                    }
                });
            }
        }
        ...
        Dates.each(S('#laydate_ys li'), function (i, elem) {
            if (Dates.checkVoid(elem.getAttribute('y'))[0] === 'y') {
                ...
                Dates.on(elem, 'click', function (ev) {
                    ...
                    Dates.viewDate(this.getAttribute('y') | 0, Dates.ymd[1], Dates.ymd[2], true);
                });
            }
        });
    };
    ...
    Dates.initDate = function (format) {
        ...
        Dates.inymd = ymd;
        //请求节假日数据到变量Dates.HolidaysData中，Modify=masj,Date=2017-06-09
        if (Dates.options.isShowHoliday && Dates.options.isShowHoliday == true && config.showHolidayRequestURL) {
            if (!Dates.HolidaysData) {
                $.ajaxSettings.async = false;//先改成同步请求，待请求完之后，再改回异步请求
                $.getJSON(config.showHolidayRequestURL, function (data) {
                    if (data && data.Result) {
                        Dates.HolidaysData = data.Result;
    ...
    Dates.iswrite = function () {
        ...
        Dates.shde(as.ok, !('issure' in Dates.options ? Dates.options.issure : 0));
    };
    ...
    Dates.view = function (elem, options) {
        ...
        Dates.iswrite();
        Dates.options.isInitCheck = ('isInitCheck' in Dates.options ? Dates.options.isInitCheck : true);
        if (Dates.options.isInitCheck) {
            Dates.check();
        }
    };
    ...
    //返回最终日期
    Dates.creation = function (ymd, hide, holidayName) {
        ...
        if (!hide) {
            Dates.close();
            typeof Dates.options.choose === 'function' && Dates.options.choose(getDates, val);
        }
    };
    ...
    Dates.events = function () {
        ...
        Dates.on(S(log.box), 'click', function (ev) {
            //当鼠标在选择时分秒层中，点击右上角的X时 Modify=masj,Date=2017-03-23
            if (ev && ev.target && $(ev.target).parent('div.laydte_hsmtex:visible').size() > 0) {
                Dates.reshow();
            }
            //Modify=masj,Date=2017-03-23
            if (log.box != '#laydate_box') {
                Dates.reshow();
            }
        });
        ...
        log.tabYear = function (type) {
            ...
            if (type < 2) {
                Dates.viewDate(Dates.ymd[0], Dates.ymd[1], Dates.ymd[2], true);
                Dates.reshow();
            } else {
                Dates.viewYears(log.YY);
            }
        ...
        Dates.each(S('#laydate_ms span'), function (i, elem) {
            Dates.on(elem, 'click', function (ev) {
                ...
                if (!Dates.hasClass(this, as[1])) {
                    Dates.viewDate(Dates.ymd[0], this.getAttribute('m') | 0, Dates.ymd[2], true);
                }
            });
        });
        ...
        Dates.each(S('#laydate_table td'), function (i, elem) {
            Dates.on(elem, 'click', function (ev) {
                $('div.laydate_box > table.laydate_table > tbody > tr > td[y][m][d]').removeClass(as[6]);
                if (!Dates.hasClass(this, as[6])) {
                    Dates.stopmp(ev);
                    Dates.addClass(this, as[6]);
                }
                if (!Dates.hasClass(this, as[1])) {
                    Dates.stopmp(ev);
                    Dates.ymd = [this.getAttribute('y') | 0, (this.getAttribute('m') | 0) > 0 ? (this.getAttribute('m') | 0) - 1 : 0, this.getAttribute('d') | 0];

                    Dates.creation([this.getAttribute('y') | 0, this.getAttribute('m') | 0, this.getAttribute('d') | 0], Dates.options.isNeedConfirm, $(elem).attr('h'));
                }
            });
        });
        ...
        Dates.on(as.otoday, 'click', function () {
            ...
            Dates.viewDate(now.getFullYear(), now.getMonth(), now.getDate());
            $('div.laydate_box > table.laydate_table > tbody > tr > td[y="' + now.getFullYear() + '"][m="' + (now.getMonth() + 1) + '"][d="' + now.getDate() + '"]').addClass(as[6]);
            Dates.reshow();
            Dates.RenderVoidDateRange();
            Dates.RenderSelectedPoint();

            // 2016-09-23 18:20:54 修复选中今天choose方法得不到数据
            Dates.creation([now.getFullYear(), now.getMonth() + 1, now.getDate()], Dates.options.isNeedConfirm);
        ...
        Dates.on(as.ok, 'click', function () {
            if (Dates.valid) {
                //Dates.creation([Dates.ymd[0], Dates.ymd[1] + 1, Dates.ymd[2]]);
                var obj = $('div.laydate_box > table.laydate_table > tbody > tr > td.laydate_curClick[y][m][d]');
                if (obj.size() == 1) {
                    Dates.creation([obj.attr('y'), obj.attr('m'), obj.attr('d')], null, obj.attr('h'));
                }
                else {
                    Dates.msg('请选择一个日期！');
                }
        ...
        log.hmson = function (input, index) {
            ...
            var nextInput = $(input).parent().next().find(':input:first');
                ...
                if (set) {
                    ...
                    Dates.on(elem, 'click', function (ev) {
                        if (!Dates.hasClass(this, as[1])) {
                            ...
                            //在选择时分秒层中，点击相应的内容后，弹出下一个层，或取消当前层 Modify=masj,Date=2017-03-23
                            if (nextInput && nextInput.size() > 0) {
                                nextInput.click();
                            }
                            if (nextIndex >= 3) {
                                Dates.stopmp(input).reshow();
                            }
                        }
                    });
                }
