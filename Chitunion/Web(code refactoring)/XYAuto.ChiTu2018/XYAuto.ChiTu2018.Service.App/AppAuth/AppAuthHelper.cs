...
namespace XYAuto.ChiTu2018.Service.App.AppAuth
{
    public class AppAuthHelper
    {
        ...
        /// </summary>
        private string ChiTuApp_API_AppIDSecret = XYAuto.CTUtils.Config.ConfigurationUtil.GetAppSettingValue("ChiTuApp_API_AppIDSecret", false);
        /// <summary>
        /// 当前配置签名列表
        /// </summary>
        ...
        public AppAuthHelper()
        {
            ...
            {
                appAuthList = null;
                XYAuto.CTUtils.Log.Log4NetHelper.Default().Error("解析Json配置[ChiTuApp_API_AppIDSecret]出错", ex);
            }
        }
        ...
        public bool VerifySign(Dictionary<string, object> list, ref EnumAppAuthRequestVerify code)
        {
            try
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    else if (name == "timestamp")
                    {
                        ...
                        DateTime? dt = XYAuto.CTUtils.Sys.ConverHelper.ConvertDateTimeByTimeStamp(currentTimestamp);
            ...
            {
                XYAuto.CTUtils.Log.Log4NetHelper.Default().Error("验证APP调用后端API接口签名异常", ex);
                code = EnumAppAuthRequestVerify.签名错误;
                return false;
            }
