...
using XYAuto.ChiTu2018.Service.LuckDrawActivity;
    ...
    public class LuckDrawController : ApiController
    {
        ...
        public JsonResult GetDrawRemainder()
        {
            ...
            try
            {
                ...
                dic = LuckDrawService.Instance.GetDrawRemainder(userId);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetActivityBasicInfo()
        {
            ...
            {
                var userId = ITSC.Chitunion2017.Common.UserInfo.GetLoginUserID();
                dic = LuckDrawService.Instance.GetActivityBasicInfo(userId);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetAwardRecord(int pageIndex = 1, int pageSize = 20)
        {
            ...
            {
                var userId = ITSC.Chitunion2017.Common.UserInfo.GetLoginUserID();
                dic = LuckDrawService.Instance.GetAwardRecord(userId, pageIndex, pageSize);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetAwardeeList()
        {
            ...
            try
            {
                ...
                dicList = LuckDrawService.Instance.GetAwardeeMoniList();
            }
            catch (Exception ex)
            {
        ...
        public JsonResult LotteryDraw()
        {
            try
            {
                var userId = ITSC.Chitunion2017.Common.UserInfo.GetLoginUserID();
                int errorMsg = 0;
                var dicDraw = LuckDrawService.Instance.LotteryDraw(userId, out errorMsg);
                if (errorMsg < 0)
                {
                    return Common.Util.GetJsonDataByResult(null, errorMsg == -1 ? "完成签到后才可参与抽奖活动哦~" : "抽奖失败！请稍后重试", errorMsg);
                }
                dicDraw = LuckDrawService.Instance.GetTaskList(dicDraw);
