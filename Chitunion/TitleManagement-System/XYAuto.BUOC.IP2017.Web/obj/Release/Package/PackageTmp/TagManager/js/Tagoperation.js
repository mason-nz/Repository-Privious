...
function DelimitAndSelected() {
    ...
    $('.detail_mark li').off('click').on('click', function () {// 点击选中，再点取消选中
        ...
        if ($(this).attr('class') != 'hasChecked') {
            ...
            if ($(this).parents('ul').attr('id') == 'classification') {
                // 审核和修改不做限制
                if ($('#classification li.hasChecked').length >= 5&&GetRequest().SelectType==undefined) {
                    layer.msg('最多选择5个分类');
                    return false;
                }
            }
            if ($(this).parents('ul').attr('id') == 'ip') {
                // 审核和修改不做限制
                if ($('#ip li.hasChecked').length >= 5&&GetRequest().SelectType==undefined) {
                    layer.msg('最多可选择5个IP');
                    return false;
                }
            }
            ...
            if ($(this).attr('name') == 'selected') {//判断是否是ip的
                var IpId=[]
                $.ajax({
                    url: labelApi.tag+'/api/MediaLabel/GetSubIPByPID',
                    type: 'get',
                    data: {
                        dictID: $(this).attr('dictid')
                    },
                    dataType: 'json',
                    xhrFields: {
    ...
    // input
    $('input:not(.TaskCount1,.TaskCount)').attr('placeholder','请输入标签文字，标签之间用 ";" 分隔')
}
