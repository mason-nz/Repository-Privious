$(function () {
    ...
    ToExamine.prototype = {
        ...
        right: function () {
            var _this=this;
            setAjax({
                url: labelApi.tag+'/api/CarSerialLabel/RenderBatchCar',
                // url:'json/Labeltypecar.json',
                type: 'GET',
                data: {
                    BrandID: GetRequest().BrandID,
                    SerialID:GetRequest().SerialID
                }
            }, function (data) {
                var result = {};
                if (data.Status == 0) {
                    ...
                    var SerialName=result.Result.SerialName?'/'+result.Result.SerialName:'';
                    $('.container .mark_right').before('<ul style="margin: 15px 0 0 0;margin-bottom: -25px;">'+
                        '<li class="wechat_name" style="margin-left: 62px;">'+
                        '<div><b style="font-size: 16px;">品牌／车型：</b>'+
                        result.Result.BrandName+SerialName
                        +'</div>'+
                        '</li>'+
                        '<div class="clear"></div>'+
                        '</ul>');
        ...
        Submit:function () {
            ...
            $('#Submit').off('click').on('click',function () {
                ...
                $.ajax({
                    url:labelApi.tag+'/api/CarSerialLabel/BatchCarSubmit',//  json/info.json
                    type:'post',
                    ...
                    success: function (data){
                        if(data.Status == 0){
                            layer.msg('成功',{time:1000}, function () {
                                window.location = '/TagManager/carLabelList.html?isSearch=1';
                            })
                        }else{
