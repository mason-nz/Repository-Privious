...
{
    public class ImportTouTiao : Import
    {
        public readonly static ImportTouTiao Instance = new ImportTouTiao();

        private readonly int importUserID = int.Parse(ConfigurationManager.AppSettings["CarMapIP_UserID"]);
        public void ImportTouTiaoLabel(int CategoryCount, int MarkCount, int XyCount, int IpCount, string excelPath)
        {
            Loger.Log4Net.Info("[ImportTouTiaoLabel]程序开始运行开始...");
            Loger.Log4Net.Info("[ImportTouTiaoLabel]Excel位置:" + excelPath);
            Loger.Log4Net.Info("[ImportTouTiaoLabel]统一导入的UserID：" + importUserID);
            if (!File.Exists(excelPath))
            {
                Loger.Log4Net.Error($"[ImportTouTiaoLabel]导入文件[{excelPath}]不存在");
                return;
            }
            DataSet ds = ConvertExcelToDataSet(new string[] { "头条媒体" }, excelPath);
            if (ds == null && ds.Tables.Count == 0)
            {
                Loger.Log4Net.Error($"[ImportTouTiaoLabel]导入文件[{excelPath}][头条媒体]sheet页不存在");
                return;
            }

            DataView dataView = ds.Tables["头条媒体"].DefaultView;
            DataTable dt = dataView.ToTable();
            if (dt.Rows.Count == 0)
            {
                Loger.Log4Net.Error($"[ImportTouTiaoLabel]导入文件[{excelPath}][头条媒体]没有数据");
                return;
            }
            ...
            DataTable dtMediaTouTiao = XYAuto.BUOC.IP2017.Dal.MediaBase.MediaBase.Instance.SelectMediaTouTiao();
            List<MediaTouTiao> listTouTiao = Util.DataTableToList<MediaTouTiao>(dtMediaTouTiao);
            if (listTouTiao == null)
            {
                Loger.Log4Net.Error($"[ImportTouTiaoLabel]未找到头条信息");
                return;
            }
            int InsertError = 0;
            int InsertSuccess = 0;
            ...
            int RepeatModel = 0;
            int irow = 1;
            MediaTouTiao mediaTouTiao;
            foreach (DataRow row in dt.Rows)
            {
                Loger.Log4Net.Info($"[ImportTouTiaoLabel]便历行：{++irow}");
                string Name = "";
                if (string.IsNullOrWhiteSpace(row["媒体地址"].ToString()))
                {
                    continue;
                }
                else
                {
                    ...
                    mediaTouTiao = listTouTiao.Where(t => t.UserName == Name).FirstOrDefault();
                    if (mediaTouTiao == null)
                    {
                        mediaTouTiao = new MediaTouTiao();
                        mediaTouTiao.UserName = Name;
                        mediaTouTiao.Url = row["媒体地址"].ToString().Trim();
                        int ResultMedia = XYAuto.BUOC.IP2017.Dal.MediaBase.MediaBase.Instance.InsertMediaTouTiao(mediaTouTiao, importUserID);
                        if (ResultMedia <= 0)
                        {
                            InsertError++;
                            Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体信息媒体Name：{Name}插入失败");
                            continue;
                        }
                        for (int i = 1; i <= CategoryCount; i++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(dicName))
                            {
                                ...
                                if (tbModel != null)
                                {
                                    ...
                                    {
                                        Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体Name[{Name}]的分类[{dicName}]在插入失败");
                                    }
                                }
                                else
                                    Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体Name[{Name}]的分类[{dicName}]在分类中未找到");
                            }
                        }
                        InsertSuccess++;
                    }
                }
                int resultID = XYAuto.BUOC.IP2017.Dal.Result.ResultLabel.Instance.GetResultID((int)EnumTaskType.媒体, -2, mediaTouTiao.Url, (int)EnumMediaType.头条);
                if (resultID <= 0)
                {
                    resultID = ResultLabel.Instance.InsertResult((int)EnumTaskType.媒体, -2, -2, (int)EnumMediaType.头条, "", mediaTouTiao.UserName, mediaTouTiao.HeadImg, mediaTouTiao.Url, importUserID);
                }
                if (resultID <= 0)
                {
                    Loger.Log4Net.Error($"[ImportTouTiaoLabel]模板文件媒体Name：[{Name}]插入失败");
                    continue;
                }
                InsertModel++;
                ...
                {
                    Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体Name：[{Name}]未插入市场场景");
                }
                #endregion
                #region 分发场景
                for (int i = 1; i <= XyCount; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(dicName))
                    {
                        ...
                        if (tbModel != null)
                        {
                            ...
                            Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体Name：[{Name}]的分发场景[{dicName}]在标签库未找到");
                    }
                }
                ...
                {
                    Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体Name：[{Name}]未插入分发场景");
                }
                #endregion
                #region IP
                    ...
                    if (!string.IsNullOrWhiteSpace(ip))
                    {
                        ...
                        if (tbModel == null)
                        {
                            ...
                            if (newIpID > 0)
                            {
                                ...
                                tbModel.Name = ip;
                                listIP.Add(tbModel);
                                Loger.Log4Net.Info($"[ImportTouTiaoLabel]母IP：[{ip}]插入成功");
                            }
                            else
                            {
                                Loger.Log4Net.Error($"[ImportTouTiaoLabel]母IP：[{ip}]插入失败");
                                continue;
                            }
                        }
                                    ...
                                    if (!string.IsNullOrWhiteSpace(subips))
                                    {
                                        ...
                                        if (tbModelSonIp == null)
                                        {
                                            ...
                                            if (newSonIpID > 0)
                                            {
                                                ...
                                                Loger.Log4Net.Info($"[ImportTouTiaoLabel]子IP：[{subip}]插入成功");
                                            }
                                            else
                                            {
                                                Loger.Log4Net.Error($"[ImportTouTiaoLabel]子IP：[{subip}]插入失败");
                                                continue;
                                            }
                                        }
                                            ...
                                            foreach (var varLabel in label.Split(new string[] { "、" }, StringSplitOptions.None))
                                            {
                                                if (!string.IsNullOrWhiteSpace(varLabel))
                                                {
                                                    ...
                                                    if (LabelID <= 0)
                                                    {
                                                        ...
                                                        if (newLabelID <= 0)
                                                        {
                                                            Loger.Log4Net.Error($"[ImportTouTiaoLabel]标签：[{varLabel.Trim()}]插入失败");
                                                            continue;
                                                        }
                                        ...
                                        {
                                            Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体[{Name}]下子IP[{ tbModelSonIp.Name}]无标签");
                                            continue;
                                        }
                                    }
                                }
                        ...
                        {
                            Loger.Log4Net.Error($"[ImportTouTiaoLabel]媒体[{Name}]下IP标签[{IpInfo.DictName}]无子IP");
                        }
                    }
                }
                #endregion
                ...
                {
                    RepeatModel++;
                    Loger.Log4Net.Info($"[ImportTouTiaoLabel]媒体[{Name}]完全重复");
                    continue;
                }
                XYAuto.BUOC.IP2017.Dal.Result.ResultLabel.Instance.InsertResultLabel(auditLabel, resultID, importUserID, false);
            }

            Loger.Log4Net.Info($"[ImportTouTiaoLabel]插入媒体成功个数：{InsertSuccess}");
            Loger.Log4Net.Info($"[ImportTouTiaoLabel]插入媒体失败个数{InsertError}");
            Loger.Log4Net.Info($"[ImportTouTiaoLabel]插入模板个数:{InsertModel}");
            Loger.Log4Net.Info($"[ImportTouTiaoLabel]插入模板重复个数:{RepeatModel}");
            Loger.Log4Net.Info("[ImportTouTiaoLabel]程序开始运行结束...");
        }
