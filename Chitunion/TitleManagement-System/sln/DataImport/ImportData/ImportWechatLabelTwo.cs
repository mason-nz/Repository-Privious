...
{
    public class ImportWechatLabelTwo : Import
    {
        public readonly static ImportWechatLabelTwo Instance = new ImportWechatLabelTwo();
        ...
        public void ImportAppLabell(int CategoryCount, int MarkCount, int XyCount, int IpCount, int SonIpCount, string excelPath)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                {
                    RepeatModel++;
                    Loger.Log4Net.Error($"[ImportAppLabel]媒体[{Name}]完全重复");
                    continue;
                }
