...
AppReport.SJJK_YWX = (function () {
    ...
    var PingTaiStyleData = [{ id: 10001, name: '易车网PC', itemStyle: { normal: { color: '#18ECE5'} }, icon: 'image:///Images/pt_yichePC.png' },
                            ...
                            { id: 10007, name: '其他', itemStyle: { normal: { color: '#FA944A'} }, icon: 'image:///Images/pt_taoche.png' }
                            ];
    ...
    //根据业务线ID，获取业务线名称
    var GetYWXNameByID = function (id) {
        for (var i = 0; i < YeWuXianStyleData.length; i++) {
            if (YeWuXianStyleData[i].id == id) {
                return YeWuXianStyleData[i].name;
            }
        }
        return '';
    };
    ...
    var BindLeadsDetailData = function (jsonList) {
        if (jsonList && jsonList.length > 0) {
            ...
            for (var j = 0; j < jsonList.length; j++) {
                ...
                var Name = GetYWXNameByID(jsonList[j].lineId);
                ...
                var LineId = jsonList[j].lineId;
                var divObj = $('<div>').addClass('liItem');
                divObj.attr('LineId', LineId);
    ...
    var InitSwiperAndBindClickEvent = function () {
        ...
        liList.bind('click', function () {
            ...
            //初始化页面下方图表
            var ywxName = GetYWXNameByID(LineId);
            $('#spanYWX_Chart2Title,#spanYWX_Chart3Title').text(ywxName);
            GetLeads_PT_SeriesData(LineId);
    ...
    //调用接口，获取饼图——系列数据(平台)
    var GetLeads_PT_SeriesData = function (LineId) {
           ...
           // console.log(data);
            ...
            for (var i = 0; i < PingTaiStyleData.length; i++) {
                for (var j = 0; j < json.length; j++) {
                    if (json[j].id == PingTaiStyleData[i].id) {
                        var obj = {};
    ...
    //获取饼图——图例数据(平台维度)
    var GetPT_LegendData = function (dataJson) {
        var chartLegendData = [];
        for (var i = 0; i < PingTaiStyleData.length; i++) {
            for (var j = 0; j < dataJson.length; j++) {
                if (dataJson[j].id == PingTaiStyleData[i].id) {
                    var obj = {};
                    obj.name = PingTaiStyleData[i].name;
    ...
    var InitPieRingChart = function (divID, dataJson) {
        ...
        var option2 = {
            legend: {
                ...
                orient: 'vertical',
                textStyle: defaultEChartsTextStyle,
                data: GetPT_LegendData(dataJson)
            },
    ...
    var InitLineChart = function (ptID) {
        ...
        AjaxGet("/api/sjjk/GetLeadsLineChartData?r=" + Math.random(), { date: $("#spanTitleDate").html(), siteId: '0', lineId: ptID }, null,
        function (data) {
