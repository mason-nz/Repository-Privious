...
namespace BitAuto.DSC.IM_2015.BLL
{
    public class UserSendMessage
    {
        ...
        {
            return Dal.UserSendMessage.Instance.GetWOrderBusiType();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetWOrderTab(string wOrderBusiType)
        {
            int worderBusiType = 0;
            if (!string.IsNullOrEmpty(wOrderBusiType))
            {
                int.TryParse(wOrderBusiType, out worderBusiType);
            }
            return Dal.UserSendMessage.Instance.GetWOrderTab(worderBusiType);
        }
    }
}
