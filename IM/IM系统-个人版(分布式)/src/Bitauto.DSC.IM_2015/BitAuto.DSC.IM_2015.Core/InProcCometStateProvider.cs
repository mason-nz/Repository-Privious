...
namespace BitAuto.DSC.IM_2015.Core
{
    ...
    public class InProcCometStateProvider : ICometStateProvider
    {
        ...
        public CometMessage[] GetMessages(string clientPrivateToken, long lastMessageId)
        {
            ...
            var allMessages = cometClient.dicMessages.Keys.OrderBy(v => v).ToArray();

            for (int i = 0; i < allMessages.Length; i++)
            {
                cometMessage = null;
                if (cometClient.dicMessages.TryRemove(allMessages[i], out cometMessage))
                {
                    //if (cometMessage != null && cometMessage.MessageId > lastMessageId)
                    //{
                    //BLL.Loger.Log4Net.Info(string.Format("方法InProcCometStateProvider.GetMessages，找到消息,对象{0},    成功移除key:{1}，lastMessageId：{2}", clientPrivateToken, allMessages[i], lastMessageId));
                    listCometMessages.Add(cometMessage);
                    //cometClient.queueMessagesDeleted.Enqueue(cometMessage);
                    cometClient.queueMessagesDeleted.TryAdd(cometMessage.MessageId, cometMessage);
                    //}
                    //else
                    //{
                    //    BLL.Loger.Log4Net.Error(string.Format("方法InProcCometStateProvider.GetMessages，未找到消息,对象{0},  成功移除key:{1},LastMessageId：{2}.", clientPrivateToken, allMessages[i], lastMessageId));
                    //}
                }
                else
                {
                    BLL.Loger.Log4Net.Error(string.Format("方法InProcCometStateProvider.GetMessages，对象{0},    移除key{1}失败.,LastMessageId：{2}", clientPrivateToken, allMessages[i], lastMessageId));
                }
            }
        ...
        */
        public void AddMessage(string clientPrivateToken, CometMessage message)
        {
            if (this.privateCUClients.ContainsKey(clientPrivateToken))
            {
                ...
                cometClient.AddMessage(message);
            }
            else
            {
                BLL.Loger.Log4Net.Info(string.Format("方法InProcCometStateProvider.AddMessage中出错，this.privateCUClients中不包含{0}", clientPrivateToken));
            }
        }
        /// <summary>
