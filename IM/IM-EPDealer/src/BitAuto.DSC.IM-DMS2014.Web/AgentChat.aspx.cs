        ...
        private void GetAllLableWithCM()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (!dtLabel.ContainsKey(Convert.ToInt32(dr[0])))
                {
                    ...
                    sbReuslt.Append(
                        "<ul style='position: absolute; z-index: 999; left: 65px; top: -25px; border: #e0e0e0 1px solid; width: 200px; background: #FFF; border-radius: 10px; height:422px;overflow-y:auto;'>");
