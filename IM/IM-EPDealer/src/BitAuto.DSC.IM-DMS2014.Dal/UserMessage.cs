...
namespace BitAuto.DSC.IM_DMS2014.Dal
{
	...
	public class UserMessage : DataBase
	{
		...
		public DataTable GetUserMessage(QueryUserMessage query, string order, int currentPage, int pageSize, out int totalCount)
		{
            ...
            {
                strwhere += " AND a.RecID=" + query.RecID;
            }
            if (query.MemberName != Constant.STRING_INVALID_VALUE)
            {
                strwhere += " AND c.MemberName like '%" + StringHelper.SqlFilter(query.MemberName) + "%'";
            }
            if (query.LastModifyUserName != Constant.STRING_INVALID_VALUE)
            {
                strwhere += " AND d.TrueName like '%" + StringHelper.SqlFilter(query.LastModifyUserName) + "%'"; //通过存储过程决定 操作人的字段名
            }
            if (query.DistrictID != Constant.STRING_INVALID_VALUE &&query.DistrictID != "-1")
            {
                strwhere += " AND  c.District='" + StringHelper.SqlFilter(query.DistrictID) + "'";
            }
            if (query.CityGroupID != Constant.STRING_INVALID_VALUE && query.CityGroupID != "-1")
            {
                strwhere += " AND  c.CityGroup='" + StringHelper.SqlFilter(query.CityGroupID) + "'";
            }

            if (query.LastModifyUserID != Constant.INT_INVALID_VALUE && query.LastModifyUserID != -1)
            {
                strwhere += " AND  a.LastModifyUserID='" +  query.LastModifyUserID + "'";
            }
 
            if (query.Status != Constant.INT_INVALID_VALUE && query.Status != -1) 
            {
                strwhere += " AND  a.Status='" + query.Status + "'";
            }
            if (query.QueryStarttime != Constant.STRING_INVALID_VALUE)
            {
                strwhere += " AND  a.CreateTime>='" + StringHelper.SqlFilter(query.QueryStarttime) + "'";
            }
            if (query.QueryEndTime != Constant.STRING_INVALID_VALUE)
            {
                strwhere += " AND  a.CreateTime<'" + StringHelper.SqlFilter(query.QueryEndTime) + "'";
            }
            if (query.AgentID != Constant.INT_INVALID_VALUE && query.AgentID != -1)
            {
                string strcg = GetCityGroupIDByUserID(query.AgentID.ToString());
                strwhere += " AND  c.CityGroup in (" + (strcg == "" ? "'-1'" : strcg) + ")";
            }
       
			DataSet ds;

			SqlParameter[] parameters = {
					new SqlParameter("@where", SqlDbType.NVarChar, -1),
        ...
        /// 得到一个留言信息对象实体
        /// </summary>
        public BitAuto.DSC.IM_DMS2014.Entities.UserMessage GetModel(int RecID)
        {
           ...
           // BitAuto.DSC.IM_DMS2014.Entities.UserMessage model = new BitAuto.DSC.IM_DMS2014.Entities.UserMessage();
        ...
        /// </summary>
        public BitAuto.DSC.IM_DMS2014.Entities.UserMessage DataRowToModel(DataRow row)
        {
            BitAuto.DSC.IM_DMS2014.Entities.UserMessage model = new BitAuto.DSC.IM_DMS2014.Entities.UserMessage();
            if (row != null)
            {
