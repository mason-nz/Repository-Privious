...
function QueueOrderHandLer(privateToken, alias, message) {
    Chatdisabled(0);
    $("#agentAllocat").html(message.m + " 如果您不想继续等待，请<a style='cursor:pointer'  onclick='SetBeforeunload(false, onbeforeunload_handler);addMessage()'>点击留言</a>");
...
function AllocAgentForUserHandler(privateToken, alias, message) {
    ...
    $("#hidto").val(message.a);
    number = message.anum;
    $("#divagentNo").html("易车网客服 " + message.anum);
    //把分配坐席标识
    ...
    document.getElementById("Rmessages").innerHTML += "<p class='hs'>您好，欢迎光临易车网~我是您的购车博士" + message.anum + "~在接下来的时间里，我将根据您的需求，为您提供车型推荐、车型对比、车型点评、经销商及报价查询等一系列服务，请问您现在最想了解什么呢？</p>";
    //让信息框和发送信息按钮可用
    Chatdisabled(1);
    messageFlicker.show();
}
             ...
             function (msg) {
                 if (msg != "") {
                     ...
                     if (r != null && r.result != "SendToLeave" && r.result != "ClientNotExists") {//登录成功之后
                         ...
                         if (r2.result == 'sendok' && MessageType != "6") {
                             var dtime = getHoursMinute2(r2.rectime);
                             //sendmessage = replaceRegUrl(sendmessage);
                             document.getElementById("Rmessages").innerHTML += "<div class='dh2'><div class='title title2'>您 <span>：" + dtime + "</span></div><div class='dhc dhc2'>" + sendmessage + "</div></div>";
                         }
                     }
                     //坐席离线
...
function GetHMessageByAgent(privatetaken, agentid) {
    AjaxPostAsync("AjaxServers/Handler.ashx", {
        ...
        var datestr = nowdate.getFullYear() + "年" + nowdate.getMonth() + "月" + nowdate.getDay() + "日";
...
function Chatdisabled(type) {
    if (type == 0) {
        ...
        //$("#bq_listSH").click(HistroyMore);
        $("#HistroyList").bind("click", HistroyMore);
        $("#btnCapture").bind("click", jieping);
        $("#EMYiChe").bind("click", addSatisfaction);


        $("#uploadify").removeAttr("disabled");
