...
function SuccessHandler(privateToken, alias, message) {
    ...
    if (!isExistChatLayer(message.c.cs) && uid != "System") {
        ...
        var userinfo = {
            ...
            ctp: message.c.ctp,
            mn: message.c.mn,
            cst: ChatStuff.parseLongDate(message.c.cst),
            adr: message.c.adr,
            cgn: message.c.cgn,
            lct: ChatStuff.parseLongDate(message.c.lct),
            dst: message.c.dst,
            ctn: message.c.ctn,
            ctj: message.c.ctj,
            lgid: message.c.lgid,
            uft: message.c.uft,
            lmt: ChatStuff.parseLongDate(message.c.lmt)
...
function CreateUserTable(userinfo) {
    ...
    t.push('<tr><th>所属区域：</th><td>' + userinfo.cgn + '</td></tr>');
...
function HistoryClick() {
    ...
    var urlT = "MessageHistoryForAgent.aspx?LoginID=" + curr$.attr("lgid") + "&UserID=" + ChatStuff.AgentId;
    window.open(urlT);
}
...
function CheckDialogState() {
    ...
    $.ajax({
        ...
        complete: function (xhr, TS) {
            ...
            ////断网时计时判断是否超过指定时间
            try {
                if (xhr.status == 0 || xhr.status == 12029 || xhr.status == 12030 || xhr.status == 12031 || xhr.status == 12152 || xhr.status == 12159) {
                    CheckTimeOut();
                }
                else {
...
function CheckOrderComplete() {
    ...
    $.get('AjaxServers/Handler.ashx', { action: "checkorder", csid: csidT, callnum: escape(curr$.attr("ctp")), custname: escape(curr$.attr("ctn")), membercode: escape(curr$.attr("mc")) }, function (a, b, c) {
