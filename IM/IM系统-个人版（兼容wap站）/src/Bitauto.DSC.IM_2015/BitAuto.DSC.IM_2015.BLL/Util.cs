...
namespace BitAuto.DSC.IM_2015.BLL
{
    public class Util
    {
        ...
        //判断请求是否是wap站
        public static bool CheckMurl(string url)
        {
            if (string.IsNullOrEmpty(url))
            {
                return false;
            ...
            System.Uri urlstr = new Uri(url);
            string DomainName = ConfigurationUtil.GetAppSettingValue("IMDomainName");
            if (urlstr.Host.ToLower() != DomainName)
            {
                return false;
            }
            int length = url.Split('/').Length;
            string page = url.Split('/')[length - 1];
            if (page.ToLower() != "monlineservice.aspx")
            {
                return false;
            }
            return true;
