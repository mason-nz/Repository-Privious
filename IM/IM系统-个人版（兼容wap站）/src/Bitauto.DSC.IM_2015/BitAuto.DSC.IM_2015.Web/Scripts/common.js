...
function LoadingAnimation(eleId) {
    jQuery('#' + eleId).html('<div style="width:100%; height:40px;padding-top:15px;"><div class="blue-loading" style="width:50%;float:left;background-position:right;"></div><div style="float:left;padding:20px 0px 0px 10px;"></div></div>');
}
...
function MaskPage() {
    jQuery('body').append('<div name="divMaskPage" style="position: fixed; left: 0; top: 0; width: 100%; height: 100%; z-index: 20000;background-color: #666; opacity: 0.3; filter: alpha(opacity=30);"><div class="blue-loading" style="width:50%;float:left;background-position:right;"></div><div style="float:left;padding:20px 0px 0px 10px;"></div></div>');
}
function UnMaskPage() {
...
(function ($) {
    ...
    $.fn.Mask = function () {
        return this.each(function () {
            ...
            this$.append('<div name="divMaskPage__" style="position: absolute; left: 0; top: 0; right: 0;bottom: 0; z-index: 20000; background-color: #666; opacity: 0.3; filter: alpha(opacity=30);"><div class="blue-loading" style="width: 50%; float: left; background-position: right;"></div><div style="float: left; padding: 20px 0px 0px 10px;"></div></div>');
        });
    };
    $.fn.UnMask = function () {
        return this.each(function () {
        ...
        function initHtml(settings) {
            ...
            for (var i = 0; i < 28; i++) {
                if (i % 7 == 0) {
                  ...
                  yxc.push('<li ' + tS + ' onclick="BQClick(\'http://img1.baa.bitautotech.com/webpic/smilies/baabig/' + convertName(i) + '.gif\',this)"><img alt="' + yxcTitle[i] + '" title="' + yxcTitle[i] + '" src=\'http://img1.baa.bitautotech.com/webpic/smilies/baabig/' + convertName(i) + '.gif\'/></li>');
                  yxm.push('<li ' + tS + ' onclick="BQClick(\'http://img1.baa.bitautotech.com/webpic/smilies/yxm/yxm' + convert2Name(i) + '.gif\',this)"><img alt="' + yxmTitle[i] + '" title="' + yxmTitle[i] + '" src=\'http://img1.baa.bitautotech.com/webpic/smilies/yxm/yxm' + convert2Name(i) + '.gif\'/></li>');
                  oth.push('<li ' + tS + ' onclick="BQClick(\'http://img1.baa.bitautotech.com/webpic/smilies/default/' + othName[i] + '.gif\',this)"><img style=\"width:auto;height:auto;\" alt="' + othTitle[i] + '" title="' + othTitle[i] + '" src=\'http://img1.baa.bitautotech.com/webpic/smilies/default/' + othName[i] + '.gif\'/></li>');
...
//            sHtml.push('<ul class="ulBQ" id="yxm">' + yxm.join('') + '</ul></div>');
            sHtml.push('<div id="con_BQ_1" style="" did="3">');
...
function GetServerTime(dt) {
    ...
    if (!window.BTTimeNow) {
        $.post("/post.action", { action: "getservertime" }, function (data) {
            data = $.parseJSON(data);
            if (!data) {
                alert("获取服务器时间失败");
                return false;
...
function checkBrowser() {
    ...
    if (/msie/i.test(browserName) && !/opera/.test(browserName)) {
        ...
        strBrowser = "Safari: " + browserName.match(/safari.([\d.]+)/)[1];
        //return false;
...
//统计代码开始
var cnzz_protocol = (("https:" == document.location.protocol) ? " https://" : " http://");
document.write(unescape("%3Cspan id='cnzz_stat_icon_1255509543' style='display:none;' %3E%3Cscript  src='" + cnzz_protocol + "s23.cnzz.com/z_stat.php%3Fid%3D1255509543' type='text/javascript'%3E%3C/script%3E  %3C/span%3E"));
//统计代码结束