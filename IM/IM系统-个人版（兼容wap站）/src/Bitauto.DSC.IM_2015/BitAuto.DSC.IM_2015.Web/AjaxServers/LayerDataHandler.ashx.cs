...
{
    /// <summary>
    /// LayerDataHandler 的摘要说明
    /// </summary>
    public class LayerDataHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        private string PID
        {
            get
            {
                ...
                {
                    return HttpUtility.UrlDecode(HttpContext.Current.Request["PID"].ToString());
                }
                else
                {
                    return string.Empty;
        ...
        private string BGID
        {
            get
            {
                if (HttpContext.Current.Request["BGID"] != null)
                {
                    return HttpUtility.UrlDecode(HttpContext.Current.Request["BGID"].ToString());
                }
                else
                {
                    return string.Empty;
        ...
        private string BusiTypeID
        {
            get
            {
                if (HttpContext.Current.Request["BusiTypeID"] != null)
                {
                    return HttpUtility.UrlDecode(HttpContext.Current.Request["BusiTypeID"].ToString());
                }
                else
                {
                    return string.Empty;
            ...
            if (msg == "")
            {
                switch (Action)
                {
                    ...
                    case "getdistrictdata":
                        GetDistrictData(out msg);
                        break;
                    case "getmemberinfobyvisitid":
                        GetMemberInfoByVisitID(out msg);
                        break;
                    case "getcsrelateinfobycsid":
                        GetCSRelateInfoByCSID(out msg);
                        break;
                    case "getorderinfobyorderid":
                        GetOrderInfoByOrderID(out msg);
                        break;
                    ...
                    case "addonlinemessage":
                        ...
                        break;
                    case "addremarkinfo":
                        AddRemarkInfo(out msg);
                        break;
                    case "getnewmessagesdata":
                        GetNewMessagesData(out msg);
                        break;
                    case "getcontenttag":
                        GetContentTag(out msg);
                        break;
                    case "getTabInfo":
                        GetTabInfo(out msg);
                        break;
        ...
        private void GetTabInfo(out string msg)
        {
            msg = string.Empty;
            ...
            int busiTypeID = 0;
            if (!int.TryParse(BusiTypeID, out busiTypeID))
            {
                msg = "业务类型数据格式错误";
                return;
            }
            ...
            {
                msg = BLL.Util.DataTableObjectToJson(dt);
            }
        }
        private void GetContentTag(out string msg)
        {
            msg = string.Empty;
            string where = string.Empty;
            if (PID != string.Empty && BGID != string.Empty)
            {
                where += " and pid=" + StringHelper.SqlFilter(PID);
                where += " and bgid=" + StringHelper.SqlFilter(BGID);
                int pid = 0;
                int.TryParse(PID, out pid);
                int bgid = 0;
                int.TryParse(BGID, out bgid);
                msg = BLL.EmployeeAgent.Instance.GetContentTagJsonBySql(pid, bgid);
            }
        }
        private void GetNewMessagesData(out string msg)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                switch (row["Sender"].ToString())
                {
                    ...
                    case "2": newRow["newName"] = row["UserName"].ToString() + "    【" + Convert.ToDateTime(row["CreateTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss") + "】"; break;
            ...
            //msg = JavaScriptConvert.SerializeObject(newTable);
            System.Text.StringBuilder msgjson = new System.Text.StringBuilder();
            ...
            if (newTable != null && newTable.Rows.Count > 0)
            {
                ...
                {
                    msgjson.Append("{\"newName\":\"" + newTable.Rows[i]["newName"].ToString() + "\",\"Content\":\"" + newTable.Rows[i]["Content"].ToString().Replace("\"", "'") + "\",\"lastdate\":\"" + newTable.Rows[i]["lastdate"].ToString() + "\"},");
                }
                msgjson.Remove(msgjson.Length - 1, 1);
                msgjson.Append("]");
        ...
        private void GetMemberInfoByVisitID(out string msg)
        {
            ...
            DataTable dt = BLL.Conversations.Instance.GetMemberInfoByVisitID(" and a.VisitID='" + StringHelper.SqlFilter(VisitID) + "'  AND b.CSID='" + StringHelper.SqlFilter(CSID) + "'");
            if (dt.Rows.Count > 0)
            {
                msg += "{root:[";
            }
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                msg += "{VisitorName:'" + dt.Rows[i]["VisitorName"].ToString()
                     + "',VisitorType:'" + dt.Rows[i]["VisitorType"].ToString() + "'"
                     + ",ProvinceCityName:'" + dt.Rows[i]["ProvinceCityName"].ToString() + "'"
                     + ",sex:'" + dt.Rows[i]["sex"].ToString() + "'"
                     + ",phone:'" + dt.Rows[i]["phone"].ToString() + "'"
                     + ",SourceType:'" + BLL.Util.GetSourceTypeName(dt.Rows[i]["SourceType"].ToString()) + "'"
                     + ",UserReferTitle:'" + dt.Rows[i]["UserReferTitle"].ToString() + "'"
                     + ",UserReferURL:'" + dt.Rows[i]["UserReferURL"].ToString() + "'"
                     + ",ConversationStartTime:'" + dt.Rows[i]["ConversationStartTime"].ToString() + "'"
                     + ",AgentStartTime:'" + dt.Rows[i]["AgentStartTime"].ToString() + "'"
                     + ",EndTime:'" + dt.Rows[i]["EndTime"].ToString() + "'"
                     + ",Duration:'" + dt.Rows[i]["Duration"].ToString() + "'"
                     + ",AgentName:'" + dt.Rows[i]["AgentName"].ToString() + "'"
                     + ",AgentNum:'" + dt.Rows[i]["AgentNum"].ToString() + "'"
                     + ",PerSatisfaction:'" + dt.Rows[i]["PerSatisfaction"].ToString() + "'"
                     + ",ProSatisfaction:'" + dt.Rows[i]["ProSatisfaction"].ToString() + "'"
                     + ",SatisfactionContents:'" + dt.Rows[i]["SatisfactionContents"].ToString() + "'},";
            }
            if (dt.Rows.Count > 0)
            {
                msg = msg.TrimEnd(',') + "]}";
