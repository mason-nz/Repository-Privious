...
(function ($) {
    ...
    */
    $.jAlert = function (message, afterCloseCallBack, second) {
        ...
        var btnPopup_ok = '<input type="button" id="btn' + alertName + '" value="确定" onclick="javascript:$.closePopupLayer(\'' + alertName + '\');" style="min-width: 50px; height: 22px;"/>';
        ...
        if (second) {
            ...
            var timeF = setInterval(function () {
                if (--sec == 0) {
                    $.closePopupLayer(alertName);
                    clearInterval(timeF);
                } else {
                    $('#btn' + alertName).val('确定(' + sec + ')');
                }

            }, 1000);
            //setTimeout(function () { $.closePopupLayer(alertName); }, second != undefined ? second * 1000 : 1000);
        }
