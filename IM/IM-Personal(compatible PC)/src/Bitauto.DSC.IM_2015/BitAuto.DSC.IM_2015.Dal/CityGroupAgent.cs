...
namespace BitAuto.DSC.IM_2015.Dal
{
    ...
    public class CityGroupAgent : DataBase
    {
        ...
        public DataTable GetCityGroupAgent(QueryCityGroupAgent query, string order, int currentPage, int pageSize, out int totalCount)
        {
            ...
            {
                //无坐席
                sql += " AND a.District NOT IN(SELECT CityGroupID FROM CityGroupAgent WHERE Status=0)";
            }
            else if (!string.IsNullOrEmpty(query.UserID))
            {
                //存在坐席是某某人的城市群
                sql += " AND a.District IN (SELECT CityGroupID FROM CityGroupAgent WHERE Status=0 AND UserID='" + StringHelper.SqlFilter(query.UserID) + "')";
            }
        ...
        public bool DeleteCityGroupAgent(List<string> groups, List<string> agents)
        {
            string sql = @"DELETE FROM dbo.CityGroupAgent
                                    WHERE CityGroupID IN ('" + StringHelper.SqlFilter(string.Join("','", groups.ToArray())) + "') AND UserID IN ('" + StringHelper.SqlFilter(string.Join("','", agents.ToArray())) + "')";
