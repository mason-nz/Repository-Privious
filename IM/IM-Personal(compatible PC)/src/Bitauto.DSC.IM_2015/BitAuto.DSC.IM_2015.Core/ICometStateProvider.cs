...
namespace BitAuto.DSC.IM_2015.Core
{
    ...
    public interface ICometStateProvider : IDisposable
    {
        ...
        /// <param name="IISIP">消息接收者iis服务器IP</param>
        //void SendMessage(string clientPublicToken, string name, string contents, string IISIP);

        //void SendMessage(CometMessage cometMessage);

        /// <summary>
        /// 用于给对象新增消息
        /// </summary>
        void AddMessage(string clientPublicToken, CometMessage message);

        void RemoveDicMessage();
        ...
        /// <param name="name"></param>
        /// <param name="contents"></param>
        //void SendMessage(string name, string contents);
        /// <summary>
