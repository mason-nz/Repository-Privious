...
namespace BitAuto.DSC.IM_2015.MainWindowsService
{
    ...
    public class IMServices : IIMServices, IDisposable
    {
        ...
        public ProxyNetFriend[] GetAllQAndTNetFriends()
        {
            return globalManager.DicAllQAndTNetFrinds.Values.ToArray();
        }
            ...
            if (globalManager.DicAllNetFriends != null)
            {
                if (globalManager.DicAllNetFriends.ContainsKey(NetFriendKey))
                {
                    ...
                    if (netfriend != null)
                    {
                        ...
                        netfriend.IsAgentReply = true;
                    }
                }
            }
            if (globalManager.DicAllQAndTNetFrinds != null)
            {
                if (globalManager.DicAllQAndTNetFrinds.ContainsKey(NetFriendKey))
                {
                    ProxyNetFriend netfriend = globalManager.DicAllQAndTNetFrinds[NetFriendKey];
                    if (netfriend != null)
                    {
                        netfriend.ConverSTime = DateTime.Now;
                        netfriend.CreateTime = DateTime.Now;
        ...
        ////更新网友实体，坐席回复时间
        //public void UpdateNetFriend(ProxyNetFriend newNetFrind)
        //{
        //    if (globalManager.DicAllQAndTNetFrinds != null && newNetFrind != null)
        //    {
        //        if (globalManager.DicAllQAndTNetFrinds.ContainsKey(newNetFrind.Token))
        //        {
        //            ProxyNetFriend netfriend = globalManager.DicAllQAndTNetFrinds[newNetFrind.Token];
        //            netfriend = newNetFrind;
        //        }
        //    }
        //}
        ...
        public bool IsNetFrindExists(string token)
        {
            return globalManager.DicAllQAndTNetFrinds.ContainsKey(token);
        }
        public ProxyNetFriend GetNetFrind(string token)
        {
            if (globalManager.DicAllQAndTNetFrinds.ContainsKey(token))
            {
                return globalManager.DicAllQAndTNetFrinds[token];
            }
            return null;
        }
        /// <summary>
        ...
        public void UpdateClientPostion(string token, string ipStr, int Type)
        {
            if (token.IndexOf('@') > 0)
            {
                int _token = 0;
                int.TryParse(token.Split(',')[0], out _token);
                ProxyAgentClient agentClient = GetAgentByToken(_token);
                if (agentClient != null)
                {
                    agentClient.IISIP = ipStr;
                }
                else
                {
                    ProxyNetFriend netFrindClient = GetNetFrind(token);
                    if (netFrindClient != null)
                    {
                        netFrindClient.IISIP = ipStr;
                    }
                    if (globalManager.DicAllNetFriends != null)
                    {
                        if (globalManager.DicAllNetFriends.ContainsKey(token))
                        {
                            ProxyNetFriend netfriend = globalManager.DicAllNetFriends[token];
                            if (netfriend != null)
                            {
                                netfriend.IISIP = ipStr;
                            }
                        }
                    }
                }
        ...
        /// 0 成功，-1：已经存在，-2：添加失败
        /// </returns>
        public int AddNetFrind(ProxyNetFriend netFrind)
        {
            if (netFrind == null) { return -2; }

            if (globalManager.DicAllQAndTNetFrinds.ContainsKey(netFrind.Token))
            {
                return -1;
            }
            if (globalManager.DicAllQAndTNetFrinds.TryAdd(netFrind.Token, netFrind))
            {
                //agent.LastActiveTime = DateTime.Now;
                BLL.Loger.Log4Net.Info(string.Format("AddNetFrind添加网友{0}", netFrind.Token));
                return 0;
            }
            else
            {
        ...
        public void RemoveAgent(int agentId, int closeType)
        {
            ...
            if (globalManager.DicAllAgents.ContainsKey(agentId))
            {
                ...
                //删除对象，同时删除它的消息
                if (globalManager.DicIPMessagePool.ContainsKey(agent.AgentToken))
                {
                    ConcurrentDictionary<long, CometMessage> deleteMessage = null;
                    globalManager.DicIPMessagePool.TryRemove(agent.AgentToken, out deleteMessage);
                    deleteMessage = null;
                }
                BLL.AgentStatusDetail.Instance.UpdateAgentLastStatus(agent.AgentStatusRecID);
        ...
        public CometMessage[] SwitchMessage(string ip, CometMessage[] messages)
        {
            ...
            ProxyNetFriend[] netFrindArray = GetAllQAndTNetFriends();
            for (int i = 0; i < netFrindArray.Length; i++)
            {
                ProxyNetFriend netFrind = netFrindArray[i];
                if (netFrind.IISIP == ip)
                {
                    tokenList.Add(netFrind.Token);
                }
            }
            ProxyAgentClient[] agentArray = GetAllAgents();
            for (int i = 0; i < agentArray.Length; i++)
            {
                ProxyAgentClient agentClient = agentArray[i];
                if (agentClient.IISIP == ip)
                {
                    tokenList.Add(agentClient.AgentToken);
                }
            }
            List<CometMessage> cometMessageList = new List<CometMessage>();
            for (int i = 0; i < tokenList.Count; i++)
            {
                //2.从指定IP消息队列中取所有消息            
                string token = tokenList[i];
                if (globalManager.DicIPMessagePool.ContainsKey(token) && !globalManager.DicIPMessagePool[token].IsEmpty)
                {
                    var resultList = globalManager.DicIPMessagePool[token].ToArray();
                    try
                    {
                        ...
                        {
                            CometMessage nT = valuePair.Value;

                            if (nT != null)
                            {
                                cometMessageList.Add(nT);
                            }
                        }
                    }
                    catch (Exception ex)
            ...
            if (cometMessageList != null && cometMessageList.Count > 0)
            {
                return cometMessageList.ToArray();
            }
            return null;
        ...
        /// <summary>
        /// 根据同步过来的要删除的消息id,删除消息
        /// </summary>
        /// <param name="tokenAndMessageIDs"></param>
        public void DeleteHistroyMessage(string[] tokenAndMessageIDs)
        {
            if (tokenAndMessageIDs != null && tokenAndMessageIDs.Length > 0)
            {
                for (int i = 0; i < tokenAndMessageIDs.Length; i++)
                {
                    try
                    {
                        string token = tokenAndMessageIDs[i].Split(',')[0];
                        long _messageID = 0;
                        long.TryParse(tokenAndMessageIDs[i].Split(',')[1], out _messageID);
                        BLL.Loger.Log4Net.Info("iis传过来的消息：" + tokenAndMessageIDs[i]);
                        if (globalManager.DicIPMessagePool.ContainsKey(token))
                        {
                            for (long deleteID = 0; deleteID <= _messageID; deleteID++)
                            {
                                if (globalManager.DicIPMessagePool[token].ContainsKey(deleteID))
                                {
                                    CometMessage delete = null;
                                    globalManager.DicIPMessagePool[token].TryRemove(deleteID, out delete);
                                    BLL.Loger.Log4Net.Info("iis主动删除消息，消息ID：" + deleteID);
                                }
                            }
                        }

                        int _agentID = 0;
                        if (int.TryParse(token.Split('@')[0], out _agentID))
                        {
                            //如果是坐席
                            if (globalManager.DicAllAgents.ContainsKey(_agentID))
                            {
                                globalManager.DicAllAgents[_agentID].LastRequestTime = DateTime.Now;
                            }
                        }
                    ...
                    {
                        BLL.Loger.Log4Net.Error(string.Format("IMServices.DeleteHistroyMessage中出错,                 {0}", ex.Message));
                    }
        ...
        /// <param name="agentStatusRecID">当前状态对应的数据库状态表主键</param>
        /// <returns> 0 成功，-1：不存在，-2：添加失败</returns>
        public int SetAgentStatus(int agentid, int status, int agentStatusRecID)
        {
            if (globalManager.DicAllAgents.ContainsKey(agentid))
            {
                ...
                globalManager.DicAllAgents[agentid].AgentStatusRecID = agentStatusRecID;
            ...
            try
            {
                ...
                foreach (ProxyNetFriend netFriend in agent.TalkUserList.Values.ToArray())
                {
                    ...
                    if (globalManager.DicAllNetFriends.ContainsKey(netFriend.Token))
                    {
                        ...
                        {
                            globalManager.DicAllQAndTNetFrinds.TryRemove(netFriend.Token, out netFrienddelete);
                            //删除对象，同时删除它的消息
                            if (globalManager.DicIPMessagePool.ContainsKey(netFriend.Token))
                            {
                                ConcurrentDictionary<long, CometMessage> deleteMessage = null;
                                globalManager.DicIPMessagePool.TryRemove(netFriend.Token, out deleteMessage);
                                deleteMessage = null;
                            }
                            netFrienddelete = null;
                        }
        ...
        private ProxyNetFriend RemoveNetFriendFromList(string token)
        {
            ...
            if (globalManager.DicAllQAndTNetFrinds.ContainsKey(token))
            {
                ProxyNetFriend netFrienddelete = null;
                globalManager.DicAllQAndTNetFrinds.TryRemove(token, out netFrienddelete);
                netFrienddelete = null;
            }
            //删除对象，同时删除它的消息
            if (globalManager.DicIPMessagePool.ContainsKey(token))
            {
                ConcurrentDictionary<long, CometMessage> deleteMessage = null;
                globalManager.DicIPMessagePool.TryRemove(token, out deleteMessage);
                deleteMessage = null;
            }
            return netFriend;
        }
        ...
        public int TransferNetFriend(int fromAgent, int toAgent, string netFriendToken)
        {
            try
            {
                ...
                UserActionLog SourceLog = new UserActionLog()
                {
                    ...
                    OperUserType = 1,
                    LogInType = 10, //转出
                    LogInfo = string.Format("坐席 {0} 转出网友 {1} 给坐席{2}. ", fromAgent, netFriendToken, toAgentClient.AgentID)
                };
                UserActionLog TagertLog = new UserActionLog()
                {
                    ...
                    OperUserType = 1,
                    LogInType = 9, //转入
                    LogInfo = string.Format("坐席 {0} 从坐席{1}转入网友 {2}", toAgentClient.AgentID, fromAgent, netFriendToken)
                };
            ...
            try
            {
                ...
                foreach (string strToken in globalManager.DicAllNetFriends.Values.Where(w => w.IISIP == iisIP).Select(s => s.Token).ToArray())
                {
                    if (!dicIISNetFriends.ContainsKey(strToken))
                    {
                        ...
                        globalManager.DicAllQAndTNetFrinds.TryRemove(strToken, out pef);
