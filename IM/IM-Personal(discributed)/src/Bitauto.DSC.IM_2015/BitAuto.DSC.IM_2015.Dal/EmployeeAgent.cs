...
namespace BitAuto.DSC.IM_2015.Dal
{
	...
	public class EmployeeAgent : DataBase
	{
        ...
        //public DataTable GetContentTagJsonBySql(string where)
        //{
        //    string strSql = "select * from WorkOrderTag where status=0 " + where + " order by OrderNum asc";
        //    return  SqlHelper.ExecuteDataset(ConnectionStrings_CC, CommandType.Text, strSql).Tables[0];
        //}
        /// <summary>
        /// 根据父节点ID，以及业务类型取标签，一级标签PID=0
        /// </summary>
        /// <param name="pID"></param>
        /// <param name="busiTypeID"></param>
        /// <returns></returns>
        public DataTable GetContentTagJsonBySql(int pID,int busiTypeID)
        {
            string strSql = "select RecID,BusiTypeID,TagName from dbo.WOrderTag where PID=" + pID + " and BusiTypeID=" + busiTypeID + " and Status!=-1 order by SortNum asc";
            return SqlHelper.ExecuteDataset(ConnectionStrings_CC, CommandType.Text, strSql).Tables[0];
        }
    }
}
