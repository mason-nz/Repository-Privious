...
namespace BitAuto.DSC.IM_2015.Web.AjaxServers
{
    ...
    public class Handler : IHttpHandler, IRequiresSessionState
    {
        ...
        private void init(HttpContext context, out string msg)
        {
            ...
            //    if (!string.IsNullOrEmpty(FromPrivateToken))
            //    {
            //        var channel = DefaultChannelHandler.StateManager.GetWCFClient();
            ...
            try
            {
                ...
                if (UserType == (Int32)Entities.UserType.Agent)
                {
                    ...
                    var cometT = DefaultChannelHandler.StateManager.InitializeClient_new(FromPrivateToken, strFromToken, BLL.Util.GetLoginRealName(), connectionTimeoutSeconds, connectionIdleSeconds, 0, UserType, "", "", "", "", "", "", IsReInit, out errormsg);
                    if (cometT == null)
                    {
        ...
        private void AgentQuit(out string msg)
        {
            ...
            if (cometClient != null)
            {
                DefaultChannelHandler.StateManager.AgentQuit(cometClient);
            }
        ...
        {
            msg = string.Empty;
            CometClient cometAgentClient = DefaultChannelHandler.StateManager.GetCometClient(FromPrivateToken);
            try
            {
                DefaultChannelHandler.StateManager.AgentRemoveNetFriend(cometAgentClient, SendToPrivateToken);
            }
            catch (Exception ex)
            {
                //msg = "没找到网友。";
        ...
        private void NetFriendQuit(out string msg)
        {
            ...
            try
            {
                ...
                if (cometClient != null)
                {
                    ...
                    DefaultChannelHandler.StateManager.NetFriendQuit(cometClient);
                }
            }
            catch (Exception ex)
            {
        ...
        private void SetAgentState(out string msg)
        {
            ...
            try
            {
                ...
                var nResult = DefaultChannelHandler.StateManager.SetAgentState(cometClient.PrivateToken, AgentState);
                if (nResult == -1)
                {
                    msg = "没找到客服。";
                    msg = "{\"result\":\"" + msg + "\"}";
                    ...
                    if (comet != null)
                    {
                        ...
                        if (listWait != null)
                        {
                            for (int i = 0; i < listWait.Count; i++)
                            {
                                ...
                                if (netFriend.Token == comet.PrivateToken)
                                {
                                    isWait = true;
                                    break;
                                }
