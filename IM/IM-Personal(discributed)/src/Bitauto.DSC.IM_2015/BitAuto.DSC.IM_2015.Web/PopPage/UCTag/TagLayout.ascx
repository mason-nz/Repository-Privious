...
<script type="text/javascript">
    ...
    function setTabPop(obj, cursel, n) {
        var this$ = $(obj);
    ...
    function checkLeft(obj) {
        var this$ = $(obj);
        var ulTagC$ = this$.siblings('li');
        if (ulTagC$.length <= 11) {
            return false;
        };

        var rr$ = this$.siblings('li[rr=1]'); // $('#ulTagC >li[rr=1]');
        var ll$ = this$.siblings('li[ll=1]'); //$('#ulTagC >li[ll=1]');
        var rn$ = ulTagC$.siblings('.r_arrow');
    ...
    function toggleUl(obj) {
        $(obj).toggleClass("toggle").closest('.bq1').next().next().find('.Tab2').toggle();
