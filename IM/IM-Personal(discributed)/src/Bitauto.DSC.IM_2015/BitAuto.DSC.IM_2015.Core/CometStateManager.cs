...
namespace BitAuto.DSC.IM_2015.Core
{
    ...
    public class CometStateManager : IDisposable, IIsMsgCallBackServices
    {
        ...
        public int EnQueueWaitAgent(string businessLine, CometClient netFriendClient)
        {
            try
            {
                var channel = GetAutoWCFClient();
                channel.AddNetFriendWaitList(new ProxyNetFriend()
                {
                    ...
                    AgentToken = "",
                    contractphone = netFriendClient.Userloginfo.Phone,
                    ConverSTime = new DateTime(1900, 1, 1),
                    CreateTime = new DateTime(1900, 1, 1),
                    CSID = -1,
                    IISIP = netFriendClient.IPIISWY,
                    IsTurnOut = false,
                    NetFName = netFriendClient.DisplayName,
                    Token = netFriendClient.PrivateToken,
                    VisitID = netFriendClient.Userloginfo.VisitID
        ...
        public void SendMessage(CometMessage cometMessage, int sentType = 0)
        {
            try
            {
                ...
                var netFriendClient = GetCometClient(cometMessage.ToToken);
        ...
        */
        ...
        public void NoticeNetFriendRemoved(CometClient netFriend, int CloseType)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(netFriend.AgentToken))
                {
                    ...
                    channel.RemoveNetFriendFromWaitList(netFriend.BusinessLines, netFriend.PrivateToken);

                    //更新网友队列放弃时间
                    if (netFriend.Userloginfo != null)
                    {
                        BLL.UserVisitLog.Instance.UpdateQueueFailTime(netFriend.Userloginfo.VisitID);
                    }
                ...
                {
                    //坐席网友不在同一个服务器上,调用接口删除网友
                    channel.RemoveNetFriend(netFriend.PrivateToken, false, true, CloseType);
                    return;
                }
        ...
        public void NoticeAgentRemoved(CometClient agentClient, int closeType)
        {
            if (agentClient == null)
            { return; }
            try
        ...
        public void AgentRemoveNetFriend(CometClient agentClient, string strNetFriendToken)
        {
            ...
            try
            {
                this.cometmessagedeal.ForceSyncMessage();
                //将网友从坐席对话列表中删除。
                this.StateProvider.RemoveSingleUser(strNetFriendToken, agentClient.PrivateToken);

                var channel = GetAutoWCFClient();

                var netFriendClient = GetCometClient(strNetFriendToken);
                if (netFriendClient == null)
                {
                    //非本地网友，给网友发离线消息
                    channel.RemoveNetFriend(strNetFriendToken, true, false, (int)Entities.CloseType.AgentClose);
                    return;
                }
                ...
                KillIdleCometClient(netFriendClient);
        ...
        /// </summary>
        /// <param name="agentClient"></param>
        public void AgentQuit(CometClient agentClient)
        {
            try
            {
                ...
                KillWaitRequest(agentClient.PrivateToken);
                KillIdleCometClient(agentClient);
            }
            catch (Exception ex)
            {
        ...
        public void NetFriendQuit(CometClient netFriendClient)
        {
            ...
            try
            {
                ...
                KillWaitRequest(netFriendClient);
                KillIdleCometClient(netFriendClient);
            }
            catch (Exception ex)
            {
        ...
        public void KillIdleCometClient(CometClient cometClient)
        {
            if (cometClient != null && Interlocked.CompareExchange(ref cometClient.IsDeleted, 1, 0) == 0)
            {
                ...
                {
                    BLL.Loger.Log4Net.Info(string.Format("CometStateManager.KillIdleCometClient中异常：{0}", ex.Message));
                }
            }
        }
        ...
        public int SetAgentState(string clientPrivateToken, AgentStatus state)
        {
            ...
            if (state == AgentStatus.Leaveline)
            {
                //NoticeAgentRemoved(GetCometClient(clientPrivateToken));
                RemoveLocalAgentTalkUsers(GetCometClient(clientPrivateToken));
            }
            try
            {
                var channel = GetAutoWCFClient();
                return channel.SetAgentStatus(Convert.ToInt32(GetCometClient(clientPrivateToken).AgentID), (int)state);
            }
            catch (Exception ex)
            {
        ...
        */
        ...
        {
            this.cometmessagedeal.ForceSyncMessage();
            var fromClient = GetCometClient(strSourceAgent);
            //从当前坐席中移除网友
