    ...
    public class CommonModel
    {
        ...
        public void InitAgent()
        {
            ...
            try
            {
                ...
                if (sResult.IndexOf("fm") > 0)
                {
                    this.AgentID = sResult.Substring(sResult.IndexOf("fm") + 5,
                        (sResult.Length - sResult.IndexOf("fm") - 7));
                    this.strPrivateToken = this.AgentID;
                }
                SetAgentStatus();
                SendLongConnet();
            }
        ...
        public void InitWY(int isAgent)
        {
            ...
            try
            {
                ...
                {
                    strPrivateToken = sResult.Substring(sResult.IndexOf("loginid") + 10,
                        sResult.IndexOf("\"", sResult.IndexOf("loginid") + 10) - sResult.IndexOf("loginid") - 10); //sResult.Substring(31, 36);
                    cominquene();
                    SendLongConnet();
                }
            }
        ...
        public void SendLongConnet()
        {
            ...
            if (IsAgent == 1)
            {
                HttpWebResponseUtility.CreatePostHttpResponseAsync("http://im.sys1.bitauto.com/poll",
        ...
        private void InitCSIDStuff(string strResult)
        {
            if (strResult.IndexOf("MAllocAgent") > 0)
            {
                ...
                AgentID = strResult.Substring(nS + 8, nE - nS - 10);

                //nS = strResult.LastIndexOf("\"cs\"");
                nS = strResult.LastIndexOf("\\\"cs\\\"");
                nE = strResult.IndexOf(",", nS);
                CSID = strResult.Substring(nS + 7, nE - nS - 7);
            }
        }
