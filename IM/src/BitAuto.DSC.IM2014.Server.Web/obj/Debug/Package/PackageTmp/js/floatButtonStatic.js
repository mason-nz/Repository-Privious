...
(function () {
    ...
    function getIconParams(bitIM_Config) {
        var params = [], queryParam = [];
        if (typeof bitIM_Config == "undefined" || isJson(bitIM_Config) == false) {
            return false;
        }
        ...
        var rs = {
            ...
            "float": typeof params["floatToRight"] != "undefined" ? params["floatToRight"] : 1,
            "side": typeof params["floatSide"] != "undefined" ? params["floatSide"] : 5,
            "top": typeof params["floatTop"] != "undefined" ? params["floatTop"] : 150,
            "close": typeof params["closeIcon"] != "undefined" ? params["closeIcon"] : 0,
            "online": typeof params["online"] != "undefined" ? params["online"] : ""//,
            //"offline": typeof params["live800_offline"] != "undefined" ? params["live800_offline"] : ""
        };
        if (typeof params["operator"] != "undefined") {
            rs["operator"] = params["operator"];
        }
        if (typeof params["skill"] != "undefined") {
            rs["skill"] = params["skill"];
        }
        return rs;
    };
    function iconGenerator(bitIM_Config) {
        var icons = document.getElementById("divBitAutoIM");
        if (!icons) {
            var pms = getIconParams(bitIM_Config);
            if (!pms) {
                return false;
            }
            var divhtmls = "<div class='lim_float_icon' id='divBitAutoIM' lim_company='' lim_float='" + pms["float"] + "' lim_close='" + pms['close'] + "' lim_side='" + pms["side"] + "' lim_top='" + pms["top"] + "' lim_online='" + pms["online"] + "' lim_offline=''";
            ...
            icons = document.getElementById("divBitAutoIM");
        }
        var items = [];
        items.push(icons);
        return items;
    ...
    window.openChat = function (el) {
        ...
        if (typeof live800_chatVersion != "undefined" && live800_chatVersion == 5)
    ...
    function iconProcess(bitIM_Config) {
        setSessionPageReferrer();
        setSessionFirstEnterUrl();
        var icons = iconGenerator(bitIM_Config), pm, iconClose;
        if ((!icons || icons.length == 0) && loadicontimes < 10) {
            loadicontimes++;
            iconProcess(bitIM_Config);
            return;
        }
        if (!icons) {
            return;
        ...
        var item, imageUrl, imageHtml;
        //var urlPrefix = protocol + live800_baseUrl + live800_baseWebApp + "/SurferServer?cmd=111";
        ...
        for (var i = 0; i < icons.length; i++) {
            ...
            if (item.getAttribute("lim_online")) {
                ...
                imageUrl += unescape(item.getAttribute("lim_online"));
            }
...
//            }
            imageHtml += "<div style='position:relative;z-index:2147483647;'><a target='_blank' href='" + bitIM_EntryChatURL + "' " + pm + "><img src='" + imageUrl + "' border='0' style='cursor:pointer;'  /></a></div>";
...
//            }
    ...
    //初始化参数
    var loadicontimes = 0;
    var bitIM_EntryChatURL = "http://im.sys1.bitauto.com/UserChat.aspx";
    if (isIE6) {
        checkdocumentready(bitIM_Config);
    } else {
        iconProcess(bitIM_Config);
    }
