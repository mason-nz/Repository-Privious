...
function AspNetComet(handler, privateToken, alias) {
    ...
    //排队到达上限通知
    this.MaxQueueHandLers = new Array();
...
AspNetComet.prototype.addMaxQueueHandLer =
function AspNetComet_addMaxQueueHandLer(func) {
    this.MaxQueueHandLers[this.MaxQueueHandLers.length] = func;
}
...
AspNetComet.prototype.callMaxQueueHandLer =
function AspNetComet_callMaxQueueHandLer(privateToken, alias, message) {
    for (var i = 0; i < this.MaxQueueHandLers.length; i++) {
        this.MaxQueueHandLers[i](privateToken, alias, message);
    }
}
                    ...
                    switch (message.n) {
                        ...
                        default:
                            ...
                            for (var i = result.length; i > 0; i--) {
                                ...
                                else if (message.n == "MaxQueue") {
                                    aspNetComet.callMaxQueueHandLer(aspNetComet.privateToken, aspNetComet.alias, message.c);
                                }
                                else {
