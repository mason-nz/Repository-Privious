...
function CreateDialogDiv(userinfo) {
    ...
    t.push('</div>');
    t.push('<div class="endmsg">(发送快捷键：Enter)最后收到消息时间：<em></em><span class="right btn" style="*margin-top: -20px;"><input type="button" value="结束对话" title="结束对话" class="w80 gray endBtn" /><input type="button" value="新增工单" style="margin:0 2px;"  title="新增工单" class="w80 gray" /><input type="button" value="发送" name="btnSend" title="发送" class="w60" /></span></div>');
    t.push('</div>');
    return t.join('');
}
...
function BindUserListClick() {
    $('#tabLeft').click(function (eve) {
        ...
        if (eve.target.nodeName == "A") {
            ...
            if ($.jConfirm("您确定要关闭本网友对话么？", function (r) {
                if (r) {
...
function TransferAgent() {
    ...
    function tAgentHandler(targetAgentId, agentNumber) {
        ...
        MaskPage();
        $.post("AjaxServers/Handler.ashx", { action: "transferagent", FromPrivateToken: ChatStuff.AgentId, targetagent: targetAgentId, wyid: ChatStuff.UserId }, function (data) {
            UnMaskPage();
            data = $.parseJSON(data);
            if (data.result == "0") {
...
function HistoryClick() {
    ...
    var urlT = "ConversationHistory.aspx?vid=" + curr$.attr("vid");
    window.open(urlT);
}
...
function CheckOrderComplete() {
    var csidT = ChatStuff.CsId;
    $('#MainContentDiv').Mask();
    $.post('AjaxServers/Handler.ashx', { action: "checkorder", csid: csidT }, function (a, b, c) {
        ...
        if (CustID) {
            urlT.push(ChatStuff.EditCustBaseInfoUrl);
            urlT.push("?CallType=3&CustID=" + CustID + "&ctype=4&SYSType=isIM2&CSID=" + csidT + "&r=" + Math.random());
        } else {

            urlT.push(ChatStuff.WorkOrderUrl);
            urlT.push("?CallType=3&IsClientOpen=1&CalledNum=" + ChatStuff.ctp + "&SYSType=isIM2&ctype=4");
