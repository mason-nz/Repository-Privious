...
{
    /// <summary>
    /// 易湃部门提供加解密类库，用于易湃后台跳转到IM系统，获取登录经销商信息
    /// </summary>
    public class EP_DESEncryptor
    {
        #region 私有成员
        /// <summary>
        /// 输入字符串
        ...
        public EP_DESEncryptor(string eKey)
        {

            if (eKey.Length != 8) throw new FormatException("密钥必须为8位");

            // 设置固定的密钥
            this.encryptKey = this.decryptKey = eKey;
        }
        #endregion
        ...
        public string DesDecrypt(string _inputString)
        {
        ...
        /// <param name="this.outFilePath">输出文件路径</param>
        /// <param name="decryptKey">密钥</param>
        public void FileDesDecrypt(string _inputFilePath, string _outFilePath)
        {
            this.inputFilePath = _inputFilePath;
            this.outFilePath = _outFilePath;
            FileDesDecrypt();
