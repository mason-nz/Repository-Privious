    ...
    public class Handler : IHttpHandler, IRequiresSessionState
    {
        ...
        private void checkorder(out ReturnMessage returnModel)
        {
            ...
            if (string.IsNullOrEmpty(cs.OrderID))
            {
                ...
                BLL.Loger.Log4Net.Info(string.Format("调用CC经销商二手车版添加工单接口，参数callNum:{0},csID:{1},custName:{2},cbsex:{3},cbprovinceID:{4},cbCityID:{5},cbCounty:{6},memberCode:{7}", CallNum, CsId, CustName, -1, -1, -1, -1, MemberCode));
                wOrderURL = CCWebServiceHepler.Instance.CCDataInterface_GetAddWOrderComeIn_IMJXS_SC_URL(CallNum, CsId, CustName, -1, -1, -1, -1, MemberCode,1);
                BLL.Loger.Log4Net.Info(string.Format("调用CC经销商二手车版添加工单接口,返回值：{0}", wOrderURL));
