<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CSHistoryData.aspx.cs"
    Inherits="BitAuto.DSC.IM_DMS2014.Web.AjaxServers.ContentManage.CSHistoryData" %>


    <asp:repeater runat="server" id="Rt_CSHistoryData">
        <ItemTemplate>
            <div class="dh1">
                <div class="title">
                  <%#Eval("newName").ToString()%>
                </div>
                <div class="dhc">
                    <%#Eval("Content").ToString()%>
                </div>
            </div>
        </ItemTemplate>
    </asp:repeater>
    <br />
    <!--分页-->
    <div class="pageTurn mr10" style=" width:480px;">
        <div class="pagesnew" style="float: right;" id="itPage">
           <p>
            <asp:literal runat="server" id="litPagerDown_History"></asp:literal>
           </p>
        </div>
    </div>
    <input type="hidden" value="<%=RecordCount %>" id="hidHistoryTotalCount" />
