...
function StartTimer() {
    ...
    ChatStuff.checkStateTimer = setInterval(CheckDialogState, ChatStuff.checkStateDuration);
}
...
function CheckDialogState() {
    ...
    $('#tabLeft > div[CsId]').each(function (i, e) {
        //cslst.push($(e).attr('did'));
        if (!$(e).hasClass('offline')) {
            cslst.push($(e).attr('CsId'));
        }
    });
    if (cslst.length == 0) return;
    $.get('AjaxServers/Handler.ashx', { action: "checkstate", csids: cslst.join(',') }, function (a, b, c) {
        var s = $.parseJSON(a);
        if (s.result == "sendok") {

            $.each(s.data, function (n, val) {
...
function CheckOrderComplete() {
    ...
    $.get('AjaxServers/Handler.ashx', { action: "checkorder", csid: csidT }, function (a, b, c) {
        ...
        urlT.push(escape(curr$.find('h4').text().trim()));
        urlT.push("&CSID=");
