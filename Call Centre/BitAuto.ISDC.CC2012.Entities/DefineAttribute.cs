...
namespace BitAuto.ISDC.CC2012.Entities
{
    ...
    public class AttributeHelper
    {
        ...
        /// <param name="info"></param>
        /// <param name="id"></param>
        public static void SetIDENTITY<T>(T info, object id)
        {
            ...
            foreach (string key in dic.Keys)
            {
                if (dic[key].IDENTITY)
                {
                    if (dic[key].DBType == SqlDbType.Int)
                    {
                        dic[key].ProInfo.SetValue(info, int.Parse(id.ToString()), null);
                    }
                    else if (dic[key].DBType == SqlDbType.BigInt)
                    {
                        dic[key].ProInfo.SetValue(info, long.Parse(id.ToString()), null);
                    }
                    break;
                }
            }
