<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CustomerFields.aspx.cs"
    Inherits="BitAuto.ISDC.CC2012.Web.CustBaseInfo.CustomerFields" %>

<script type="text/javascript">
    $(document).ready(function () {
        $.get("/CustBaseInfo/CustomerFields.xml", LoadCheckFromXML);
        $(".allSelect").click(function () {
            if ($(this).attr("checked") == true) {
                $("#ckDiv_" + $(this).attr("typename")).find("input[type=checkbox]").attr("checked", true);
            }
            else {
                $("#ckDiv_" + $(this).attr("typename")).find("input[type=checkbox]").attr("checked", false);
            }
        });
    ...
    function ReturnFidldStr() {
        ...
        $("#hidFieldsCustomer").val(fieldStr);
        ...
        SetCookie("CustomerFields", selectIds);
        $("#hidIsOkOrCancel").val("1");
        $.closePopupLayer("DisposeSetPoper");
    }

    function CookieSet() {
        var c = GetCookie("CustomerFields");
