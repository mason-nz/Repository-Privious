<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TagLayer.aspx.cs" Inherits="BitAuto.ISDC.CC2012.Web.WOrderV2.PopLayer.TagLayer" %>

<script type="text/javascript">
    var tp = {};
    tp.BusiTypeID = '<%=BusiTypeID %>';
    tp.PID = '<%=PID %>';
    tp.Level = '<%=Level %>';
    ...
    tp.SearchGroup = function () {
        //状态
        var status = "";
        if ($('#ckAvalibile20').is(':checked')) {
            status += $('#ckAvalibile20').attr("value") + ",";
        }
        if ($('#ckAvalibile21').is(':checked')) {
            status += $('#ckAvalibile21').attr("value") + ",";
        }
        if (status.length > 0) {
            status = status.substring(0, status.length - 1);
        }

        var pody = "status=" + status + "&busitypeid=" + tp.BusiTypeID + "&pid=" + tp.PID + "&level=" + tp.Level + "&r=" + Math.random();

        $('#divList').load('PopLayer/TagLayer.aspx #divList > *', pody, function () { });
    ...
    tp.saveTr = function (obj) {
        ...
        if (dataid == "-1") {
            body = { action: "InsertNewTag", currid: dataid, tagname: cn, currsort: currDataSort, pid: tp.PID, busitypeid: tp.BusiTypeID, r: Math.random() };
        } else {
            body = { action: "ChangeName", currid: tr$.attr('dataid'), tagname: cn, pid: tp.PID, currsort: currDataSort, r: Math.random() };
        }
        ;
        tp.SendReqest(body, function (data) {
            if (dataid == "-1") {
                ;
                tr$.attr("dataid", data.RecID); //id
                tr$.attr("datasort", data.SortNum); //排序
            }
    ...
    tp.SendReqest = function (body, callback) {
        ...
        AjaxPostAsync("../../WOrderV2/Handler/TagHandler.ashx", body, null, function (data) {
            if (data) {
                ;
    ...
    $(function () {
        ...
        $("#sel_busiTypeTag").change(function () {
            tp.BusiTypeID = $("#sel_busiTypeTag").val();
            tp.SearchGroup();
        });
                ...
                if (data.Success == true) {
                    ...
                    if (jsonData != null) {
                        for (var i = 0; i < jsonData.length; i++) {
                            $("#sel_busiTypeTag").append("<option value=" + jsonData[i].Id + ">" + jsonData[i].Name + "</option>");
                        }

                    }

                    if (tp.BusiTypeID) {
                        $("#sel_busiTypeTag").val(tp.BusiTypeID);
                    }
                    else {
                        tp.BusiTypeID = $("#sel_busiTypeTag").val();
                    }
...
<div class="pop pb15 openwindow" style="width: 600px;">
    <div class="title bold">
        <h2>
            标签管理</h2>
        <span><a href="javascript:void(0)" onclick="javascript:$.closePopupLayer('TagLayer',false);">
        </a></span>
    </div>
    <div class="search">
        <ul class="clearfix  outTable" style="width: 98%;">
            ...
            <li style="float: left; width: 200px;">
                <label style="width: 80px;">
                    业务类型：</label><span>
                        <select id="sel_busiTypeTag" class="w100">
                        </select></span></li>
            <%} %>
            <li style="width: 180px; float: left;" class="name1">
                <label style="width: 80px;">
                    状态：</label>
                <input type="checkbox" name="chkStatus" id="ckAvalibile20" value="1" checked="checked"
                    onclick="javascript:tp.SearchGroup();" />在用
                <input type="checkbox" name="chkStatus" id="ckAvalibile21" value="0" onclick="javascript:tp.SearchGroup();" />停用
            </li>
        </ul>
    </div>
                            ...
                            <td style=" text-align:left; padding-left:5px;">
                                <span class="tagname">
                                    <%#Eval("TagName")%></span> <span class="inputname" style="display: none;">
                                        <input type="text" class="vInput w150" value="<%#Eval("TagName")%>" />
                                    </span>
                            </td>
                            <td>
    ...
    <div class="btn">
        ...
        <%--<input type="button" class="btnSave bold" onclick="javascript:$.closePopupLayer('TagLayer',false);"
            value="关闭" />--%>
    </div>
    <div class="clearfix">
    </div>
