<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UCSurveyInfoEdit.ascx.cs"
    Inherits="BitAuto.ISDC.CC2012.Web.ReturnVisit.UCSurveyInfoEdit" %>
...
<script type="text/javascript">
    ...
    function MustRadio(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false && $('#' + pre_arrRadio[i]).attr("must") == "1") {
                emptystr += "第" + RadioNoNumber + "题单选题必答。<br/>";
            }
    ...
    function MustCheck(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false && $('#' + pre_arrRadio[i]).attr("must") == "1") {
                emptystr += "第" + RadioNoNumber + "题多选题必答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function MustText(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false && $('#' + pre_arrRadio[i]).attr("must") == "1") {
                emptystr += "第" + RadioNoNumber + "题文本题必答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function MustMatrixRadio(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false && $('#' + pre_arrRadio[i]).attr("must") == "1") {
                emptystr += "第" + RadioNoNumber + "题矩阵单选题必答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function MustMatrixSelect(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false && $('#' + pre_arrRadio[i]).attr("must") == "1") {
                emptystr += "第" + RadioNoNumber + "题矩阵评分题必答。<br/>";
            }
        }
        return emptystr;
    }
            ...
            $("input[type='radio'][name='" + pre_arrRadio[i] + "'][typename='" + asktype + "']").each(function () {
                ...
                if ($(this).attr("checked")) {
                    if ($("#" + $(this).val()).attr("showflag") == "1") {
                        if ($("#" + $(this).val()).val() == "") {
                            $("#" + $(this).val()).focus();
                            emptystr += "第" + $(this).parents("li[nums]").attr("nums") + "题请在选项后输入文本。<br/>";
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (maxlong != "" && maxlong != "0") {
                if (checkednum > parseInt(maxlong)) {
                    emptystr += "第" + $('#' + pre_arrRadio[i]).attr("index") + "题多选题，选择项数不能大于" + maxlong + "项。<br/>";
                }
            }
            if (minlong != "" && minlong != "0") {
                if (checkednum < parseInt(minlong)) {
                    emptystr += "第" + $('#' + pre_arrRadio[i]).attr("index") + "题多选题，选择项数不能小于" + minlong + "项。<br/>";
                }
            }
        }
        return emptystr;
                ...
                if ($(this).val() != "") {
                    ...
                    if ($(this).attr("maxLong") != "" && $(this).attr("minLong") != "") {
                        if (strlen($(this).val()) > parseInt($(this).attr("maxLong"))) {
                            $(this).focus();
                            stralter += "第" + $(this).parents("li[nums]").attr("nums") + "题，文本输入长度不能大于最大允许输入长度" + parseInt($(this).attr("maxLong")) + "个字符。<br/>";
                        }
                        if (strlen($(this).val()) < parseInt($(this).attr("minLong"))) {
                            $(this).focus();
                            stralter += "第" + $(this).parents("li[nums]").attr("nums") + "题，文本输入长度不能小于最小允许输入长度" + parseInt($(this).attr("minLong")) + "个字符。<br/>";
                        }
                    }
                }
            });
    ...
    function CheckMatrixRadio(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (j != 0) {
                ...
                if (hang > 0 && hang > j) {
                    stralter += "第" + num + "为矩阵单选题，如若作答请全部作答，为了保证统计结果的准确度，请返回填写。<br/>";
                }
            }
        }
        return stralter;
    ...
    function CheckMatrixDrop(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (j != 0) {
                ...
                if (hang > 0 && hang > j) {
                    stralter += "第" + num + "为矩阵评分题，如若作答请全部作答，为了保证统计结果的准确度，请返回填写。<br/>";
                }
            }
        }
        return stralter;
    ...
    function emptyRadio(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false) {
                emptystr += "第" + RadioNoNumber + "题单选题没答。<br/>";
            }
    ...
    function emptyCheck(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false) {
                emptystr += "第" + RadioNoNumber + "题多选题没答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function emptyText(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false) {
                emptystr += "第" + RadioNoNumber + "题文本题没答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function emptyMatrixRadio(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false) {
                emptystr += "第" + RadioNoNumber + "题矩阵单选题没答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function emptyMatrixSelect(pre_arrRadio) {
        ...
        for (var i = 0; i < pre_arrRadio.length; i++) {
            ...
            if (flag == false) {
                emptystr += "第" + RadioNoNumber + "题矩阵评分题没答。<br/>";
            }
        }
        return emptystr;
    }
    ...
    function CheckDataForSurvey() {
        ...
        //验证通过
        if (emptystr != "") {
            if (confirm(emptystr + "<br/>提交后此问卷不可再编辑，确认提交？")) {
                return true;
            }
            else {
                return false;
                                        ...
                                        <ItemTemplate>
                                            <li <%# GetChecked(Eval("SQID").ToString(),Eval("SOID").ToString(),"","")=="checked"?"class='chooseXx'":""%>>
                                                <input name="<%#Eval("SQID")%>" typename="<%=Convert.ToString((Int32)BitAuto.ISDC.CC2012.Entities.AskCategory.RadioT)%>"
                                                    ...
                                                    <%#GetChecked(Eval("SQID").ToString(),Eval("SOID").ToString(),"","")%> value="<%#Eval("SOID")%>"
                                                    class="dt" /><span style="cursor: pointer" onclick="emChkIsChoose(this)" yes='<%#GetJump(Eval("SOID").ToString())%>'
                                                        anchor='<%#GetJump(Eval("SOID").ToString())%>'><%#Eval("OptionName")%></span><span><input
                                        ...
                                        <ItemTemplate>
                                            <li <%# GetChecked(Eval("SQID").ToString(),Eval("SOID").ToString(),"","")=="checked"?"class='chooseXx'":""%>>
                                                <input name="<%#Eval("SQID")%>" typename="<%=Convert.ToString((Int32)BitAuto.ISDC.CC2012.Entities.AskCategory.CheckBoxT)%>"
                                                    type="checkbox" <%#GetChecked(Eval("SQID").ToString(),Eval("SOID").ToString(),"","")%>
                                                    value="<%#Eval("SOID")%>" class="dt" /><span onclick="emChkIsChoose(this)" style="cursor: pointer"><%#Eval("OptionName")%><input
