...
function GooCalendar(InputID, property) {
    ...
    temp = "<div class='top'><b class='arrow'><div class='left'></div></b><b class='arrow' style='float:right;'><div class='right'></div></b></div>" +
		...
		"<div class='bottom'><div class='cal_btn'>today</div><div class='time'></div><div class='cal_btn' style='float:right'>cancel</div></div>";
    ...
    if (property.needTime) {
        ...
        if (property.BeginStyle) {
            this.$selectTime = {
                hour: $("<input type='text' size='2' maxlength='2' title='hour' value='00' onkeyup=\"if(isNaN(this.value)||this.value<0||this.value>23) this.value='0'\"/>"),
                minute: $("<input type='text' size='2' maxlength='2' title='minute' value='00' onkeyup=\"if(isNaN(this.value)||this.value<0||this.value>59) this.value='0'\"/>"),
                second: $("<input type='text' size='2' maxlength='2' title='second' value='00' onkeyup=\"if(isNaN(this.value)||this.value<0||this.value>59) this.value='0'\"/>")
            };
        }
        if (property.endStyle) {
            this.$selectTime = {
                hour: $("<input type='text' size='2' maxlength='2' title='hour' value='23' onkeyup=\"if(isNaN(this.value)||this.value<0||this.value>23) this.value='0'\"/>"),
                minute: $("<input type='text' size='2' maxlength='2' title='minute' value='59' onkeyup=\"if(isNaN(this.value)||this.value<0||this.value>59) this.value='0'\"/>"),
                second: $("<input type='text' size='2' maxlength='2' title='second' value='59' onkeyup=\"if(isNaN(this.value)||this.value<0||this.value>59) this.value='0'\"/>")
            };
        }
    ...
    //this.$week = ['Su','Mo','Tu','We','Th','Fr','Sa']; //从左至右，周日，周一。。。。。。周六
        this.$week = ['周日', '周一', '周二', '周三', '周四', '周五', '周六']; //从左至右，周日，周一。。。。。。周六
    temp = "";
    ...
    //this.$month = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']; //数组顺序，从一月至十二月
        this.$month = ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']; //数组顺序，从一月至十二月
    temp = "";
    ...
    if (!this.$fixid) {
        ...
        $("#" + InputID).bind("mousedown", { div: this.$div }, function (e) {
            ...
            locate.top += $(this).attr("offsetHeight") + 2;
            var left = locate.left;

            //判断如果浏览器是ie 7.0，且该时间控件在查询列表中 则将离左边宽度-240px，高度加3px;
            if ($.browser.msie) {
                if ($.browser.version == "7.0" && $("#sidebar").length > 0) {
                    left = left - 240;
                    locate.top = locate.top + 3;
                }
            }

            e.data.div.css({ top: locate.top + "px", left: left + "px", display: "block" });
        });
    }
    else {
        this.$InputObj = $("#" + this.$fixid);
    ...
    $(document).bind('click', { div: this.$div, fixdiv: $("#" + this.$fixid), inputObj: this.$InputObj }, function (e) {
        try {
            var DateLayer;
            if (!this.$fixid) {
                DateLayer = e.data.div[0];
            }
            else {
                DateLayer = e.data.fixdiv[0];
            }
            if ((DateLayer.style.display == "" || DateLayer.style.display == "block") &&
            arguments[0].target != e.data.inputObj[0] && $(arguments[0].target).parents('div.Calendar')[0] != DateLayer
                    ) {
